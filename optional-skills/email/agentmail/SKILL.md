---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. io@agentmail.to).
version: 1.0.0
metadata:
  io:
...
# AgentMail Φ Agent-Owned Email Inboxes

## Requirements

- **AgentMail API key** (required) Φ sign up at https://console.agentmail.to (free tier: 3 inboxes, 3,000 emails/month; paid plans from $20/mo)
...
Add to `~/.io/config.yaml` (paste your actual key Φ MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `io`)
   - The agent gets address: `io@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "io-outreach")
...
- Real-time inbound email (webhooks) requires a public server Φ use `list_threads` polling via cronjob instead for personal use

## Verification
After setup, test with:
```
io --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
