...
Connect IO to a running [NeuroSkill](https://neuroskill.com/) instance to read
...
> **ΦΦ️ Research Use Only** Φ NeuroSkill is an open-source research tool. It is
...
- Check signal quality Φ green indicators in NeuroSkill (Φ0.7 per electrode)
...
| `interactive "query"` | Cross-modal 4-layer graph search (text Φ EXG Φ labels) |
...
The `scores` object contains all live metrics (0Φ1 scale unless noted):

```jsonc
{
  "scores": {
    "focus": 0.70,           // β / (α + θ) Φ sustained attention
    "relaxation": 0.40,      // α / (β + θ) Φ calm wakefulness
    ...
    "stillness": 0.88,       // 0Φ1; 1 = perfectly still
...
Also includes: `device` (state, battery, firmware), `signal_quality` (per-electrode 0Φ1),
...
numbers alone Φ always give them meaning:

**DO:**
> "Your focus is solid right now at 0.70 Φ that's flow state territory. Heart
...
- **Focus > 0.70** Φ flow state territory, protect it
- **Focus < 0.40** Φ suggest a break or protocol
- **Drowsiness > 0.60** Φ fatigue warning, micro-sleep risk
- **Relaxation < 0.30** Φ stress intervention needed
- **Cognitive Load > 0.70 sustained** Φ mind dump or break
- **TBR > 1.5** Φ theta-dominant, reduced executive control
- **FAA < 0** Φ withdrawal/negative affect Φ consider FAA rebalancing
- **SNR < 3 dB** Φ unreliable signal, suggest electrode repositioning
...
> "Your focus started at 0.64 and climbed to 0.76 by the end Φ a clear upward trend.
...
4-layer graph: query Φ text labels Φ EXG points Φ nearby labels. Use `--k-text`,
...
Interpret comparisons with context Φ mention trends, not just deltas:
...
> but there have been more stress spikes Φ your stress index jumped 15% and
...
- **Healthy targets**: N3 15Φ25%, REM 20Φ25%, efficiency >85%, onset <20 min
...
- **> 1.5** Φ Sessions are neurally distinct (different brain states)
- **< 0.5** Φ Similar brain states across both sessions
...
> FAA is positive Φ approach motivation is engaged. Looks like a solid start."
...
session Φ silence is the correct response.
...
Always ask before starting Φ never interrupt flow state:

> "Your focus has been declining for the past 15 minutes and TBR is climbing past
> 1.5 Φ signs of theta dominance and mental fatigue. Want me to walk you through
...
Key triggers:
- **Focus < 0.40, TBR > 1.5** Φ Theta-Beta Neurofeedback Anchor or Box Breathing
- **Relaxation < 0.30, stress_index high** Φ Cardiac Coherence or 4-7-8 Breathing
- **Cognitive Load > 0.70 sustained** Φ Cognitive Load Offload (mind dump)
- **Drowsiness > 0.60** Φ Ultradian Reset or Wake Reset
- **FAA < 0 (negative)** Φ FAA Rebalancing
- **Flow State (focus > 0.75, engagement > 0.70)** Φ Do NOT interrupt
- **High stillness + headache_index** Φ Neck Release Sequence
- **Low RMSSD (< 25ms)** Φ Vagal Toning
...
npx neuroskill status --json
```
Φ Interpret scores naturally, mentioning focus, relaxation, mood, and any notable
...
npx neuroskill status --json
```
Φ Check if metrics confirm it (high theta, low beta, rising TBR, high drowsiness).
Φ If confirmed, suggest an appropriate protocol from `references/protocols.md`.
Φ If metrics look fine, the issue may be motivational rather than neurological.

**"Compare my focus today vs yesterday"**
```bash
npx neuroskill compare --json
```
Φ Interpret trends, not just numbers. Mention what improved, what declined, and
  possible causes.
...
```
Φ Report timestamps, associated metrics, and what the user was doing (from labels).

**"How did I sleep?"**
```bash
npx neuroskill sleep --json
```
Φ Report sleep architecture (N3%, REM%, efficiency), compare to healthy targets,
  and note any issues (high wake epochs, low REM).

**"Mark this moment Φ I just had a breakthrough"**
```bash
npx neuroskill label "breakthrough"
```
Φ Confirm label saved. Optionally note the current metrics to remember the state.

---

## References

- [NeuroSkill Paper Φ arXiv:2603.03212](https://arxiv.org/abs/2603.03212) (Kosmyna & Hauptmann, MIT Media Lab)
