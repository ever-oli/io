---
name: solana
description: Query Solana blockchain data with USD pricing Φ wallet balances, token portfolios with values, transaction details, NFTs, whale detection, and live network stats. Uses Solana RPC + CoinGecko. No API key required.
version: 0.2.0
author: Deniz Alagoz (gizdusum), enhanced by IO
license: MIT
metadata:
  io:
...
Helper script path: ~/.io/skills/blockchain/solana/scripts/solana_client.py
...
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py stats
```
...
```bash
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py \
...
```bash
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py price BONK
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py price JUP
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py price SOL
python3 ~/.io/skills/blockchain/solana/scripts/solana_client.py price DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263
```
...
- **Public RPC rate-limits** Φ Solana mainnet public RPC limits requests.
  For production use, set SOLANA_RPC_URL to a private endpoint
  (Helius, QuickNode, Triton).
- **NFT detection is heuristic** Φ amount=1 + decimals=0. Compressed
...
- **Transaction history** Φ public RPC keeps ~2 days. Older transactions
  may not be available.
- **Token names** Φ ~25 well-known tokens are labeled by name. Others
