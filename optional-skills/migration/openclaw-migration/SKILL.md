...
description: Migrate a user's OpenClaw customization footprint into IO. Imports IO-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: IO (Most Wanted Research)
license: MIT
metadata:
  io:
    tags: [Migration, OpenClaw, IO, Memory, Persona, Import]
    related_skills: [io]
---

# OpenClaw -> IO Migration

Use this skill when a user wants to move their OpenClaw setup into IO with minimal manual cleanup.
...
```bash
io claw migrate              # Full interactive migration
io claw migrate --dry-run    # Preview what would be migrated
io claw migrate --preset user-data   # Migrate without secrets
io claw migrate --overwrite  # Overwrite existing conflicts
io claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `io setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_io.py` to:

- import `SOUL.md` into the IO home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into IO memory entries
- merge OpenClaw command approval patterns into IO `command_allowlist`
- migrate IO-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.io/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen IO workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.io/tts/`
- archive non-secret docs that do not have a direct IO destination
...
- `scripts/openclaw_to_io.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py`

Do not guess a shorter path like `~/.io/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.io/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of IO-compatible secrets, currently:
...
IO CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in IO. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a IO workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing IO skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.io/skills/migration/openclaw-migration/scripts/openclaw_to_io.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of IO unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty IO targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean IO destination. Unsupported auth blobs must still be reported as skipped.
...
11. Do not end the dry-run summary with a vague ΦWhat would you like to do?Φ if `clarify` is available. Use structured follow-up prompts instead.
...
- IO persona state imported
- IO memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.io/skills/openclaw-imports/`
