---
name: telephony
description: Give IO phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
# Telephony Φ Numbers, Calls, and Texts without Core Tool Changes

This optional skill gives IO practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.io/.env`
...
It does **not** turn IO into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.

## Safety rules Φ mandatory
...
## Decision tree Φ which service to use?

Use this logic instead of hardcoded provider routing:

### 1) "I want IO to own a real phone number"
Use **Twilio**.
...
- pairs well with IO `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.io/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
io skills search telephony
io skills install official/productivity/telephony
```

## Provider setup

### Twilio Φ owned number, SMS/MMS, direct calls, inbound SMS polling

Sign up at:
- https://www.twilio.com/try-twilio

Then save credentials into IO:
...
### Bland.ai Φ easiest outbound AI calling
...
### Vapi Φ better conversational voice quality
...
3. Buy it and save it into `~/.io/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
This is the main answer to Φhow do I access messages the number receives next time the skill is loaded?Φ

### D. Make a direct Twilio call with built-in TTS

```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is IO calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing IO's existing `text_to_speech` support.

Use this when:
- you want the call to use IO's configured TTS voice rather than Twilio `<Say>`
...
Recommended IO TTS -> Twilio Play workflow:

1. Generate the audio with IO `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask IO to create the message audio with `text_to_speech`
...
Important note:
- IO TTS is great for prerecorded outbound messages
...
- IO STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to IO memory.
...
- Do not store arbitrary third-party phone numbers in IO memory.
...
3. persist that number to `~/.io/.env`
