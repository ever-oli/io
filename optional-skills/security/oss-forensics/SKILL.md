...
## ΦΦ️ Anti-Hallucination Guardrails
...
5. **Proof-Required Disproval**: A hypothesis cannot be dismissed without a specific, evidence-backed counter-argument. "No evidence found" is not sufficient to disproveΦit only makes a hypothesis inconclusive.
...
> resolve `SKILL_DIR` to the actual path Φ e.g. `~/.io/skills/security/oss-forensics/`
...
- Each dangling commit SHA Φ type: `git`
- Force-push evidence (reflog showing history rewrite) Φ type: `git`
- Unsigned commits from verified contributors Φ type: `git`
- Suspicious binary file additions Φ type: `git`
...
- PR exists in archive but missing from API Φ evidence of deletion
- Contributor in archive events but not in contributors list Φ evidence of permission revocation
- Commit in archive PushEvents but not in API commit list Φ evidence of force-push/deletion
...
# payload.distinct_size = 0 AND payload.size > 0 Φ force push indicator
...
2. Verify each ID exists in `evidence.json` (hard failure if any ID is missing Φ hypothesis rejected as potentially fabricated).
...
This skill is designed for **defensive security investigation** Φ protecting open-source software from supply chain attacks. It must not be used for:

- **Harassment or stalking** of contributors or maintainers
- **Doxing** Φ correlating GitHub activity to real identities for malicious purposes
- **Competitive intelligence** Φ investigating proprietary or internal repositories without authorization
- **False accusations** Φ publishing investigation results without validated evidence (see anti-hallucination guardrails)
...
- For paginated endpoints, fetch all pages in sequence Φ don't parallelize against the same endpoint
- Check `X-RateLimit-Remaining` header; if below 100, pause for `X-RateLimit-Reset` timestamp
- BigQuery has its own quotas (10 TiB/day free tier) Φ always dry-run first
...
- [github-archive-guide.md](./references/github-archive-guide.md) Φ BigQuery queries, CDX API, 12 event types
- [evidence-types.md](./references/evidence-types.md) Φ IOC taxonomy, evidence source types, observation types
- [recovery-techniques.md](./references/recovery-techniques.md) Φ Recovering deleted commits, PRs, issues
- [investigation-templates.md](./references/investigation-templates.md) Φ Pre-built hypothesis templates per attack type
- [evidence-store.py](./scripts/evidence-store.py) Φ CLI tool for managing the evidence JSON store
- [forensic-report.md](./templates/forensic-report.md) Φ Structured report template
