...
def main():
    ...
    if args.command == "add":
        ...
        if not issues:
            print(f"Φ All {len(store.data['evidence'])} evidence entries passed SHA-256 integrity check.")
        else:
            print(f"Φ {len(issues)} integrity issue(s) detected:")
            for i in issues:
