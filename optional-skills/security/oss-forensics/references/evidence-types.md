...
| `OTHER` | Catch-all for unclassified IOCs | Φ |
...
|------------|-------------------|
| `PushEvent` | Core: `payload.distinct_size=0` with `payload.size>0` Φ force push. `payload.before`/`payload.head` shows rewritten history. |
| `PullRequestEvent` | Detects deleted PRs, rapid openΦclose patterns, PRs from new accounts |
...
| `DeleteEvent` | Branch/tag deletion: critical Φ often used to hide traces |
