...
- **Do NOT** try multiple installation methods Φ pick one and proceed
...
- "Find accounts for nasa" Φ username is `nasa`
- "Search for johndoe123" Φ username is `johndoe123`
- "Check if alice exists on social media" Φ username is `alice`
- "Look up user bob on social networks" Φ username is `bob`
...
When extracting, take the **exact** username as stated Φ preserve case, numbers, underscores, etc.
...
- `--nsfw` Φ Include NSFW sites (only if user asks)
- `--tor` Φ Route through Tor (only if user asks for anonymity)

**Do NOT ask about options via clarify** Φ just run the default search. Users can request specific options if needed.
...
### No Results Found
If Sherlock finds no accounts, this is often correct Φ the username may not be registered on checked platforms. Suggest:
...
2. Username provided Φ proceed directly
...
>
> Φ https://twitter.com/johndoe123
> Φ https://github.com/johndoe123
> Φ https://instagram.com/johndoe123
> Φ [... additional links]
>
