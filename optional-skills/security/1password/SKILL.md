...
version: 1.0.0
author: arceus77-7, enhanced by IO
license: MIT
metadata:
  io:
    ...
    category: security
setup:
  help: "Create a service account at https://my.1password.com Φ Settings Φ Service Accounts"
  collect_secrets:
...
- `tmux` available for stable authenticated sessions during IO terminal calls (desktop app flow only)
...
### Service Account (recommended for IO)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.io/.env` (the skill will prompt for this on first load).
No desktop app needed. Supports `op read`, `op inject`, `op run`.

```bash
export OP_SERVICE_ACCOUNT_TOKEN="your-token-here"
op whoami  # verify Φ should show Type: SERVICE_ACCOUNT
```

### Desktop App Integration (interactive)

1. Enable in 1Password desktop app: Settings Φ Developer Φ Integrate with 1Password CLI
...
## IO Execution Pattern (desktop app flow)

IO terminal commands are non-interactive by default and can lose auth context between calls.
For reliable `op` use with desktop app integration, run sign-in and secret operations inside a dedicated tmux session.

Note: This is NOT needed when using `OP_SERVICE_ACCOUNT_TOKEN` Φ the token persists across terminal calls automatically.

```bash
SOCKET_DIR="${TMPDIR:-/tmp}/io-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/io-op.sock"
