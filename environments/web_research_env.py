"""
WebResearchEnv Φ RL Environment for Multi-Step Web Research
============================================================

Trains models to do accurate, efficient, multi-source web research.

Reward signals:
  - Answer correctness  (LLM judge, 0.0Φ1.0)
  - Source diversity    (used Φ2 distinct domains)
  - Efficiency          (penalizes excessive tool calls)
  - Tool usage          (bonus for actually using web tools)

Dataset: FRAMES benchmark (Google, 2024) Φ multi-hop factual questions
...
Inspired by: GroceryMind Φ production IO agent doing live web research
             across German grocery stores (firecrawl + io)
"""
...
SAMPLE_QUESTIONS = [
    ...
    },
    {
        ...
        "answer": "The Linux Foundation oversees Kubernetes. CNCF (Cloud Native Computing Foundation) is the specific body Φ it does not have a traditional CEO but has an executive director.",
        "difficulty": "medium",
        "hops": 2,
    },
    {
    ...
    },
    {
        ...
        "answer": "The European Central Bank sets rates for France/eurozone. The current rate should be verified Φ it has changed frequently in 2023-2025.",
        "difficulty": "hard",
        "hops": 2,
    },
    {
...
class WebResearchEnvConfig(IOAgentEnvConfig):
    ...
    diversity_bonus: float = Field(
        default=0.1,
        description="Bonus reward for citing Φ2 distinct domains.",
    )
    ...
    eval_split_ratio: float = Field(
        default=0.1,
        description="Fraction of dataset to hold out for evaluation (0.0Φ1.0).",
    )
...
class WebResearchEnv(IOAgentBaseEnv):
    """
    ...
    Reward is multi-signal:
      60% Φ answer correctness (LLM judge)
      20% Φ tool usage (did the model actually search the web?)
      20% Φ efficiency (penalizes >5 tool calls)

    Bonus +0.1 for source diversity (Φ2 distinct domains cited).
    """

    name = "web-research"
    env_config_cls = WebResearchEnvConfig

    # Default toolsets for this environment Φ web + file for saving notes
    ...
    # ------------------------------------------------------------------
    # 2. get_next_item Φ return the next question
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 3. format_prompt Φ build the user-facing prompt
    # ------------------------------------------------------------------
    ...
    def format_prompt(self, item: dict) -> str:
        ...
        return (
            ...
            f"You MUST search the web to find current, accurate information Φ "
    ...
    # ------------------------------------------------------------------
    # 4. compute_reward Φ multi-signal scoring
    # ------------------------------------------------------------------
    ...
    async def compute_reward(
        ...
        Multi-signal reward function:
          ...
          correctness_weight * correctness  Φ LLM judge comparing answer to ground truth
          tool_usage_weight  * tool_used    Φ binary: did the model use web tools?
          efficiency_weight  * efficiency   Φ penalizes wasteful tool usage
          + diversity_bonus                 Φ source diversity (Φ2 distinct domains)
        """
        ...
        logger.debug(
            f"Reward breakdown Φ correctness={correctness:.2f}, "
            f"tool_used={tool_used:.1f}, efficiency={efficiency:.2f}, "
            f"diversity={diversity:.1f} Φ total={reward:.3f}"
        )

        return reward

    # ------------------------------------------------------------------
    # 5. evaluate Φ run on held-out eval split
    # ------------------------------------------------------------------

    async def evaluate(self, *args, **kwargs) -> None:
        """Run evaluation on the held-out split using the full agent loop with tools.

        Each eval item runs through the same agent loop as training Φ
        ...
        for i, item in enumerate(eval_items):
            ...
            try:
                ...
                logger.info(
                    f"  Φ correctness={correctness:.2f}, reward={reward:.3f}, "
        ...
        logger.info(
            f"Eval complete Φ correctness={eval_metrics['eval/mean_correctness']:.3f}, "
