"""
IOAgentBaseEnv -- Abstract Base Environment for IO + Atropos

Provides the Atropos integration plumbing that all io environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via IOAgentLoop
...
# Ensure the io repo root is on sys.path so that imports like
...
# Load API keys from io/.env so all environments can access them
...
# Import io toolset infrastructure
...
class IOAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for io Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of io toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: io, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class IOAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for io Atropos integration.
    ...
    name: Optional[str] = "io"
    env_config_cls = IOAgentEnvConfig

    def __init__(
        self,
        config: IOAgentEnvConfig,
        ...
        # Set terminal environment variables so io tools pick them up.
        ...
        # for bidirectional tool call translation (raw text Φ OpenAI tool_calls).
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY io tool (terminal, file, web,
