...
Unlike TerminalBench2 (which evaluates per-task coding ability with binary pass/fail), YC-Bench measures **long-term strategic coherence** Φ whether an agent can maintain consistent strategy, manage compounding consequences, and adapt plans over hundreds of turns.
...
The environment initialises the simulation via `yc-bench sim init` (NOT `yc-bench run`, which would start yc-bench's own built-in agent loop). Our `IOAgentLoop` then drives all interaction through CLI commands.
...
- **Deterministic**: SHA256-based RNG Φ same seed + preset = same world
...
- [collinear-ai/yc-bench](https://github.com/collinear-ai/yc-bench) Φ Official repository
- [Collinear AI](https://collinear.ai/) Φ Company behind yc-bench
- [TerminalBench2](../terminalbench_2/) Φ Per-task coding benchmark (complementary)
