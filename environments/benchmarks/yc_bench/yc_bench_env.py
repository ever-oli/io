...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs IOAgentLoop with terminal tool only
...
Requires: pip install io[yc-bench]
"""
...
- **Funds**: You start with $250,000 seed capital. Revenue comes from completing
  tasks. Rewards scale with your prestige: `base × (1 + scale × (prestige Φ 1))`.
...
class YCBenchEvalConfig(IOAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends IOAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(IOAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The IOAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs IOAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our IOAgentLoop.
