...
Format uses special unicode tokens:
    <｜toolΦcallsΦbegin｜>
    <｜toolΦcallΦbegin｜>type<｜toolΦsep｜>function_name
    ```json
    {"arg": "value"}
    ```
    <｜toolΦcallΦend｜>
...
class DeepSeekV3ToolCallParser(ToolCallParser):
    ...
    START_TOKEN = "<｜toolΦcallsΦbegin｜>"

    # Updated PATTERN: Using \s* instead of literal \n for increased robustness
    # against variations in model formatting (Issue #989).
    PATTERN = re.compile(
        r"<｜toolΦcallΦbegin｜>(?P<type>.*?)<｜toolΦsep｜>(?P<function_name>.*?)\s*```json\s*(?P<function_arguments>.*?)\s*```\s*<｜toolΦcallΦend｜>",
        re.DOTALL,
    )
