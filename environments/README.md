...
```
                    ...
                    Φ       BaseEnv          Φ  (atroposlib)
                    Φ  - Server management   Φ
                    Φ  - Worker scheduling   Φ
                    Φ  - Wandb logging       Φ
                    Φ  - CLI (serve/process/ Φ
                    Φ    evaluate)           Φ
                    ΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦ
                                Φ inherits
                    ΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦ
                    Φ  IOAgentBaseEnv    Φ  io_base_env.py
                    Φ  - Terminal backend    Φ
                    Φ  - Tool resolution     Φ
                    Φ  - Agent loop          Φ
                    Φ  - ToolContext          Φ
                    Φ  - Async patches       Φ
              ...
              Φ                 Φ                  Φ
     TerminalTestEnv     IOSweEnv    TerminalBench2EvalEnv
...
**IOAgentBaseEnv** (`io_base_env.py`) extends BaseEnv with io specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves io toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `IOAgentBaseEnv` and implement:
...
`IOAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as io's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** io tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any io tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some io tools use `asyncio.run()` internally (e.g., mini-swe-agent's Modal backend via SWE-ReX). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Applied automatically at import time by `io_base_env.py`.
...
Available parsers:
- `io` -- IO/ChatML `<tool_call>` XML format
...
ΦΦΦ __init__.py                   # Package exports
ΦΦΦ io_base_env.py            # Abstract base (IOAgentBaseEnv)
...
Φ   ΦΦΦ __init__.py               # Registry + base class
Φ   ΦΦΦ io_parser.py
Φ   ΦΦΦ mistral_parser.py
Φ   ΦΦΦ llama_parser.py
Φ   ΦΦΦ qwen_parser.py
Φ   ΦΦΦ qwen3_coder_parser.py
Φ   ΦΦΦ deepseek_v3_parser.py
Φ   ΦΦΦ deepseek_v3_1_parser.py
Φ   ΦΦΦ kimi_k2_parser.py
Φ   ΦΦΦ longcat_parser.py
Φ   ΦΦΦ glm45_parser.py
Φ   ΦΦΦ glm47_parser.py
Φ
ΦΦΦ terminal_test_env/            # Stack validation environment
Φ   ΦΦΦ terminal_test_env.py
Φ
ΦΦΦ io_swe_env/               # SWE-bench style training environment
Φ   ΦΦΦ io_swe_env.py
Φ
ΦΦΦ benchmarks/                   # Evaluation benchmarks
    ΦΦΦ terminalbench_2/          # 89 terminal tasks, Modal sandboxes
    Φ   ΦΦΦ terminalbench2_env.py
...
2. Create your env file inheriting from `IOAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which io toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `io` |
