"""
IOAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the io tool-calling loop using standard OpenAI-spec tool calling.
...
identical to io's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by IOAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by IOAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class IOAgentLoop:
    """
    Runs io's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # io's standalone parsers. This handles the case where
            ...
            if assistant_msg.tool_calls:
                # Normalize tool calls to dicts Φ they may come as objects
                ...
                # Execute each tool call via io's dispatch
