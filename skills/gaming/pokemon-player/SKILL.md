---
name: pokemon-player
description: Play Pokemon games autonomously via headless emulation. Starts a game server, reads structured game state from RAM, makes strategic decisions, and sends button inputs Φ all from the terminal.
...
with a venv ready Φ just cd there and source .venv/bin/activate.

You also need a ROM file. Ask the user for theirs. On this machine
one exists at roms/pokemon_red.gb inside that directory.
NEVER download or provide ROM files Φ always ask the user.
...
The tunnel URL changes each time Φ give the user the new one if restarted.
...
### Step 1: OBSERVE Φ check state AND take a screenshot
GET /state for position, HP, battle, dialog.
GET /screenshot and save to /tmp/pokemon.png, then use vision_analyze.
Always do BOTH Φ RAM state gives numbers, vision gives spatial awareness.

### Step 2: ORIENT
- Dialog/text on screen Φ advance it
- In battle Φ fight or run
- Party hurt Φ head to Pokemon Center
- Near objective Φ navigate carefully

### Step 3: DECIDE
Priority: dialog > battle > heal > story objective > training > explore

### Step 4: ACT Φ move 2-4 steps max, then re-check
POST /action with a SHORT action list (2-4 actions, not 10-15).

### Step 5: VERIFY Φ screenshot after every move sequence
...
## Action Reference
- press_a Φ confirm, talk, select
- press_b Φ cancel, close menu
- press_start Φ open game menu
- walk_up/down/left/right Φ move one tile
- hold_b_N Φ hold B for N frames (use for speeding through text)
- wait_60 Φ wait about 1 second (60 frames)
- a_until_dialog_end Φ press A repeatedly until dialog clears
...
- Ledges, fences, signs, building doors, NPCs Φ only visible via screenshot
...
- Do not spam 10-15 movements Φ you will overshoot or get stuck
...
### Decision Tree
1. Want to catch? Φ Weaken then throw Poke Ball
2. Wild you don't need? Φ RUN
3. Type advantage? Φ Use super-effective move
4. No advantage? Φ Use strongest STAB move
5. Low HP? Φ Switch or use Potion
...
- Boulder Badge Φ Brock (Rock) Φ use Water/Grass
- Cascade Badge Φ Misty (Water) Φ use Grass/Electric
- Thunder Badge Φ Lt. Surge (Electric) Φ use Ground
- Rainbow Badge Φ Erika (Grass) Φ use Fire/Ice/Flying
- Soul Badge Φ Koga (Poison) Φ use Ground/Psychic
- Marsh Badge Φ Sabrina (Psychic) Φ hardest gym
- Volcano Badge Φ Blaine (Fire) Φ use Water/Ground
- Earth Badge Φ Giovanni (Ground) Φ use Water/Grass/Ice
- Elite Four Φ Champion!
...
- Dialog detection via RAM is unreliable Φ verify with screenshots
