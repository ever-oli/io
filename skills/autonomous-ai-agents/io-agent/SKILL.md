---
name: io-spawning
description: Spawn additional IO instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task Φ this runs a full separate io process.
version: 1.1.0
author: IO
license: MIT
metadata:
  io:
    tags: [Agent, IO, Multi-Agent, Orchestration, Subprocess, Interactive]
    homepage: https://github.com/ever-oli/io
    related_skills: [claude-code, codex]
---

# Spawning IO Instances

Run additional IO processes as autonomous subprocesses. Unlike `delegate_task` (which spawns lightweight subagents sharing the same process), this launches fully independent `io` CLI processes with their own sessions, tools, and terminal environments.

## When to Use This vs delegate_task

| Feature | `delegate_task` | Spawning `io` process |
|---------|-----------------|--------------------------|
...
- `io` CLI installed and on PATH
- API key configured in `~/.io/.env`
...
This installs uv, Python 3.11, clones the repo, sets up the venv, and launches an interactive setup wizard to configure your API provider and model. See the [GitHub repo](https://github.com/ever-oli/io) for details.
...
terminal(command="io --continue", background=true, pty=true)

# Resume a specific session by ID (shown on exit)
terminal(command="io --resume 20260225_143052_a1b2c3", background=true, pty=true)
```
...
```
terminal(command="io chat -q 'Research the latest GRPO training papers and write a summary to ~/research/grpo.md'", timeout=300)
```

Background for long tasks:
```
terminal(command="io chat -q 'Set up CI/CD for ~/myapp'", background=true)
# Returns session_id, monitor with process tool
```

## Mode 2: Interactive PTY Session

Launch a full interactive IO session with PTY for back-and-forth collaboration. You can send messages, review its work, give feedback, and steer it.

Note: IO uses prompt_toolkit for its CLI UI. Through a PTY, this works because ptyprocess provides a real terminal Φ input sent via `submit` arrives as keystrokes. The output log will contain ANSI escape sequences from the UI rendering Φ focus on the text content, not the formatting.

```
# Start interactive io in background with PTY
terminal(command="io", workdir="~/project", background=true, pty=true)
# Returns session_id
...
**Code review loop** Φ spawn io, send code for review, iterate on feedback:
```
terminal(command="io", workdir="~/project", background=true, pty=true)
...
**Research with steering** Φ start broad, narrow down based on findings:
```
terminal(command="io", background=true, pty=true)
...
**Multi-agent coordination** Φ spawn two agents working on related tasks, pass context between them:
```
# Agent A: backend
terminal(command="io", workdir="~/project/backend", background=true, pty=true)
process(action="submit", session_id="<agent-a>", data="Build a REST API for user management with CRUD endpoints")

# Agent B: frontend
terminal(command="io", workdir="~/project/frontend", background=true, pty=true)
...
```
terminal(command="io chat -q 'Research competitor landing pages and write a report to ~/research/competitors.md'", background=true)
terminal(command="io chat -q 'Audit security of ~/myapp and write findings to ~/myapp/SECURITY_AUDIT.md'", background=true)
process(action="list")
```
...
```
terminal(command="io chat -q 'Summarize this codebase' --model google/gemini-2.5-pro", workdir="~/project", background=true)
```

## Gateway Cron Integration

For scheduled autonomous tasks, use the unified `cronjob` tool instead of spawning processes Φ cron jobs handle delivery, retry, and persistence automatically.
...
```
# Start io inside tmux
tmux new-session -d -s io-session -x 120 -y 40 "io"
sleep 10  # Wait for banner/startup

# Send messages
tmux send-keys -t io-session "your message here" Enter

# Read output
sleep 15  # Wait for LLM response
tmux capture-pane -t io-session -p

# Multi-turn: just send more messages and capture again
tmux send-keys -t io-session "follow-up message" Enter

# Exit when done
tmux send-keys -t io-session "/exit" Enter
tmux kill-session -t io-session
```

## Rules

1. **Use `-q` for autonomous tasks** Φ agent works independently and exits
2. **Use `pty=true` for interactive sessions** Φ required for the full CLI UI
3. **Use `submit` not `write`** Φ `submit` adds a newline (Enter), `write` doesn't
4. **Read logs before sending more** Φ check what the agent produced before giving next instruction
5. **Set timeouts for `-q` mode** Φ complex tasks may take 5-10 minutes
6. **Prefer `delegate_task` for quick subtasks** Φ spawning a full process has more overhead
7. **Each instance is independent** Φ they don't share conversation context with the parent
8. **Check results** Φ after completion, read the output files or logs the agent produced
