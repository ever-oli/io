...
description: Free web search via DuckDuckGo Φ text, news, images, videos. No API key needed. Use the Python DDGS library or CLI to search, then web_extract for full content.
version: 1.2.0
...
**Important:** `max_results` must always be passed as a **keyword argument** Φ positional usage raises an error on all methods.
...
|------|-------------|---------|
| `-k` | Keywords (query) Φ **required** | `-k "search terms"` |
| `-m` | Max results | `-m 5` |
| `-r` | Region | `-r us-en` |
...
DuckDuckGo returns titles, URLs, and snippets Φ not full page content. To get full content, follow up with `web_extract`:
