---
name: parallel-cli
description: Optional vendor skill for Parallel CLI Φ agent-native web search, extraction, deep research, enrichment, FindAll, and monitoring. Prefer JSON output and non-interactive flows.
version: 1.1.0
author: IO
license: MIT
metadata:
...
This is an optional third-party workflow, not a IO core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with IO native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer IO native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer IO native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
ΦΦΦ search
ΦΦΦ extract / fetch
ΦΦΦ research run|status|poll|processors
ΦΦΦ enrich run|status|poll|plan|suggest|deploy
ΦΦΦ findall run|ingest|status|poll|result|enrich|extend|schema|cancel
ΦΦΦ monitor create|list|get|update|delete|events|event-group|simulate
```
...
- Do not silently choose Parallel when IO native tools are already sufficient.
