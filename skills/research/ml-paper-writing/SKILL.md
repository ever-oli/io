...
**Key Principle**: Be proactive. If the repo and results are clear, deliver a full draft. Don't block waiting for feedback on every sectionΦscientists are busy. Produce something concrete they can react to, then iterate based on their response.

---

## ΦΦ️ CRITICAL: Never Hallucinate Citations

**This is the most important rule in academic writing with AI assistance.**

### The Problem
AI-generated citations have a **~40% error rate**. Hallucinated referencesΦpapers that don't exist, wrong authors, incorrect years, fabricated DOIsΦare a serious form of academic misconduct that can result in desk rejection or retraction.

### The Rule
**NEVER generate BibTeX entries from memory. ALWAYS fetch programmatically.**

| Action | Φ Correct | Φ Wrong |
|--------|-----------|----------|
| Adding a citation | Search API Φ verify Φ fetch BibTeX | Write BibTeX from memory |
...
These are high-signal starting points for Related WorkΦthe scientist has already deemed them relevant.
...
**Never assume the narrativeΦalways verify with the human.**
...
**Be proactiveΦdeliver a complete draft rather than asking permission for each section.**
...
2. Flag specific uncertainties: "I framed X as the main contributionΦlet me know if you'd prefer to emphasize Y instead"
3. Continue with the draft rather than blocking

**Questions to include with the draft** (not before):
- "I emphasized X as the main contributionΦadjust if needed"
- "I highlighted results A, B, CΦlet me know if others are more important"
- "Related work section includes [papers]Φadd any I missed"
...
|---------|-------------------|-----------------|
| Abstract | Yes | "Framed contribution as XΦadjust if needed" |
| Introduction | Yes | "Emphasized problem YΦcorrect if wrong" |
| Methods | Yes | "Included details A, B, CΦadd missing pieces" |
| Experiments | Yes | "Highlighted results 1, 2, 3Φreorder if needed" |
| Related Work | Yes | "Cited papers X, Y, ZΦadd any I missed" |
...
**The single most critical insight**: Your paper is not a collection of experimentsΦit's a story with one clear contribution supported by evidence.
...
Copy this checklist and track progress. **Each step involves drafting Φ feedback Φ revision:**

```
Paper Writing Progress:
- [ ] Step 1: Define the one-sentence contribution (with scientist)
- [ ] Step 2: Draft Figure 1 Φ get feedback Φ revise
- [ ] Step 3: Draft abstract Φ get feedback Φ revise
- [ ] Step 4: Draft introduction Φ get feedback Φ revise
- [ ] Step 5: Draft methods Φ get feedback Φ revise
- [ ] Step 6: Draft experiments Φ get feedback Φ revise
- [ ] Step 7: Draft related work Φ get feedback Φ revise
- [ ] Step 8: Draft limitations Φ get feedback Φ revise
...
Figure 1 deserves special attentionΦmany readers skip directly to it.
...
Cite generouslyΦreviewers likely authored relevant papers.
...
**This section distills the most important writing principles from leading ML researchers.** These aren't optional style suggestionsΦthey're what separates accepted papers from rejected ones.

> "A paper is a short, rigorous, evidence-based technical story with a takeaway readers care about." Φ Neel Nanda
...
**Why?** Most reviewers form judgments before reaching your methods. Readers encounter your paper as: **title Φ abstract Φ introduction Φ figures Φ maybe the rest.**
...
|-----------|------|---------|
| **Subject-verb proximity** | Keep subject and verb close | Φ "The model, which was trained on..., achieves" Φ Φ "The model achieves... after training on..." |
| **Stress position** | Place emphasis at sentence ends | Φ "Accuracy improves by 15% when using attention" Φ Φ "When using attention, accuracy improves by **15%**" |
| **Topic position** | Put context first, new info after | Φ "Given these constraints, we propose..." |
| **Old before new** | Familiar info Φ unfamiliar info | Link backward, then introduce new |
| **One unit, one function** | Each paragraph makes one point | Split multi-point paragraphs |
| **Action in verb** | Use verbs, not nominalizations | Φ "We performed an analysis" Φ Φ "We analyzed" |
...
- **Minimize pronouns**: Φ "This shows..." Φ Φ "This result shows..."
- **Verbs early**: Position verbs near sentence start
- **Unfold apostrophes**: Φ "X's Y" Φ Φ "The Y of X" (when awkward)
...
- **Be specific**: Φ "performance" Φ Φ "accuracy" or "latency" (say what you mean)
- **Eliminate hedging**: Drop "may" and "can" unless genuinely uncertain
- **Avoid incremental vocabulary**: Φ "combine," "modify," "expand" Φ Φ "develop," "propose," "introduce"
- **Delete intensifiers**: Φ "provides *very* tight approximation" Φ Φ "provides tight approximation"
...
**ΦΦ️ IMPORTANT**: Copy the ENTIRE directory, not just `main.tex`. Templates include:
...
- Missing TeX packages Φ install via `tlmgr install <package>`
- Wrong TeX distribution Φ use TeX Live (recommended)
...
| From Φ To | Page Change | Key Adjustments |
|-----------|-------------|-----------------|
| NeurIPS Φ ICML | 9 Φ 8 pages | Cut 1 page, add Broader Impact if missing |
| ICML Φ ICLR | 8 Φ 9 pages | Can expand experiments, add LLM disclosure |
| NeurIPS Φ ACL | 9 Φ 8 pages | Restructure for NLP conventions, add Limitations |
| ICLR Φ AAAI | 9 Φ 7 pages | Significant cuts needed, strict style adherence |
| Any Φ COLM | varies Φ 9 | Reframe for language model focus |
...
When cutting pages (e.g., NeurIPS 9 Φ AAAI 7):
...
When expanding (e.g., ICML 8 Φ ICLR 9):
...
- Φ Copying `\usepackage` commands (causes conflicts)
- Φ Keeping old conference header/footer commands
- Φ Forgetting to update `\bibliography{}` path
- Φ Missing conference-specific required sections
- Φ Exceeding page limit after format change

---

## Citation Workflow (Hallucination Prevention)

**ΦΦ️ CRITICAL**: AI-generated citations have ~40% error rate. **Never write BibTeX from memory.**

### The Golden Rule

```
IF you cannot programmatically fetch a citation:
    Φ Mark it as [CITATION NEEDED] or [PLACEHOLDER - VERIFY]
    Φ Tell the scientist explicitly
    Φ NEVER invent a plausible-sounding reference
```
...
- [ ] Step 6: If ANY step fails Φ mark as placeholder, inform scientist
```
...
| Situation | Action |
|-----------|--------|
| Found paper, got DOI, fetched BibTeX | Φ Use the citation |
| Found paper, no DOI | Φ Use arXiv BibTeX or manual entry from paper |
| Paper exists but can't fetch BibTeX | ΦΦ️ Mark placeholder, inform scientist |
| Uncertain if paper exists | Φ Mark `[CITATION NEEDED]`, inform scientist |
| "I think there's a paper about X" | Φ **NEVER cite** - search first or mark placeholder |

**Φ NEVER generate BibTeX from memoryΦalways fetch programmatically. Φ**
...
- Include direction symbols (Φ higher is better, Φ lower is better)
...
- **No title inside figure**Φthe caption serves this function
- **Self-contained captions**Φreader should understand without main text
...
**Compiling to PDF:**
- **VS Code/Cursor**: Install LaTeX Workshop extension + TeX Live Φ Save to auto-compile
