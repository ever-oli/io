...
# Domain Intelligence Φ Passive OSINT
...
This skill includes `scripts/domain_intel.py` Φ a complete CLI tool for all domain intelligence operations.
...
# WHOIS lookup (registrar, dates, name servers Φ 100+ TLDs)
...
# Bulk analysis Φ multiple domains, multiple checks in parallel
...
- **crt.sh queries** use HTTPS (port 443) Φ works behind most firewalls
- **WHOIS queries** use TCP port 43 Φ may be blocked on restrictive networks
- **DNS queries** use Google DoH (HTTPS) for MX/NS/TXT Φ firewall-friendly
- **SSL checks** connect to the target on port 443 Φ the only "active" operation

## Data sources

All queries are **passive** Φ no port scanning, no vulnerability testing:

- **crt.sh** Φ Certificate Transparency logs (subdomain discovery, HTTPS only)
- **WHOIS servers** Φ Direct TCP to 100+ authoritative TLD registrars
- **Google DNS-over-HTTPS** Φ MX, NS, TXT, CNAME resolution (firewall-friendly)
- **System DNS** Φ A/AAAA record resolution
- **SSL check** is the only "active" operation (TCP connection to target:443)

## Notes

- WHOIS queries use TCP port 43 Φ may be blocked on restrictive networks
- Some WHOIS servers redact registrant info (GDPR) Φ mention this to the user
- crt.sh can be slow for very popular domains (thousands of certs) Φ set reasonable expectations
- The availability check is heuristic-based (3 passive signals) Φ not authoritative like a registrar API
