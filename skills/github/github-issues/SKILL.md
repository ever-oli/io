...
```bash
gh issue comment 42 --body "Investigated Φ root cause is in auth middleware. Working on a fix."
```
...
curl -s -X POST \
  ...
  -d '{"body": "Investigated Φ root cause is in auth middleware. Working on a fix."}'
```
