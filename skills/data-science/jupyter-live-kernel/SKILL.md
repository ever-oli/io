...
description: >
  ...
  intermediate results Φ data science, ML experimentation, API exploration, or
...
| `execute_code` | One-shot scripts needing io tool access (web_search, file ops). Stateless. |
...
1. **First execution after server start may timeout** Φ the kernel needs a moment
   to initialize. If you get a timeout, just retry.

2. **The kernel Python is JupyterLab's Python** Φ packages must be installed in
   that environment. If you need additional packages, install them into the
   JupyterLab tool environment first.

3. **--compact flag saves significant tokens** Φ always use it. JSON output can
...
5. **Argument order matters** Φ subcommand flags like `--path` go BEFORE the
...
7. **Errors are returned as JSON** with traceback Φ read the `ename` and `evalue`
   fields to understand what went wrong.

8. **Occasional websocket timeouts** Φ some operations may timeout on first try,
