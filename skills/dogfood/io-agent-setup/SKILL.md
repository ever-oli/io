...
1. **Provider selection** Φ OpenRouter, Anthropic, OpenAI, Google, DeepSeek, and many more
2. **API key entry** Φ stores securely in the env file
3. **Model selection** Φ picks from available models for the chosen provider
4. **Basic settings** Φ reasoning effort, tool preferences
...
```bash
io skills list              # Show installed skills
io skills search "docker"   # Search the hub
io skills install NAME      # Install from hub
io skills config            # Enable/disable per platform
```
...
- CLI: `io -s skill-name` or `io -s skill1,skill2`
...
1. **Local faster-whisper** Φ free, no API key, runs on CPU/GPU
2. **Groq Whisper** Φ free tier, needs GROQ_API_KEY
3. **OpenAI Whisper** Φ paid, needs VOICE_TOOLS_OPENAI_KEY
...
### Tool not available
1. Run `io tools` to check if the toolset is enabled for your platform
2. Some tools need env vars Φ check the env file
3. Use /reset after enabling tools

### Model/provider issues
1. Run `io doctor` to check configuration
2. Run `io login` to re-authenticate
...
1. Check `io skills list` shows the skill
2. Check `io skills config` has it enabled for your platform
3. Load explicitly with `/skill name` or `io -s name`
