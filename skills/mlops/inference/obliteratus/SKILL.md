---
name: obliteratus
description: Remove refusal behaviors from open-weight LLMs using OBLITERATUS Φ mechanistic interpretability techniques (diff-in-means, SVD, whitened SVD, LEACE, SAE decomposition, etc.) to excise guardrails while preserving reasoning. 9 CLI methods, 28 analysis modules, 116 model presets across 5 compute tiers, tournament evaluation, and telemetry-driven recommendations. Use when a user wants to uncensor, abliterate, or remove refusal from an LLM.
version: 2.0.0
author: IO
license: MIT
...
Remove refusal behaviors (guardrails) from open-weight LLMs without retraining or fine-tuning. Uses mechanistic interpretability techniques Φ including diff-in-means, SVD, whitened SVD, LEACE concept erasure, SAE decomposition, Bayesian kernel projection, and more Φ to identify and surgically excise refusal directions from model weights while preserving reasoning capabilities.

**License warning:** OBLITERATUS is AGPL-3.0. NEVER import it as a Python library. Always invoke via CLI (`obliteratus` command) or subprocess. This keeps IO's MIT license clean.
...
| Experimental auto-detection       | `informed`         | Auto-detects alignment type Φ experimental, may not always outperform advanced |

### 9 CLI Methods
- **basic** Φ Single refusal direction via diff-in-means. Fast (~5-10 min for 8B).
- **advanced** (DEFAULT, RECOMMENDED) Φ Multiple SVD directions, norm-preserving projection, 2 refinement passes. Medium speed (~10-20 min).
- **aggressive** Φ Whitened SVD + jailbreak-contrastive + attention head surgery. Higher risk of coherence damage.
- **spectral_cascade** Φ DCT frequency-domain decomposition. Research/novel approach.
- **informed** Φ Runs analysis DURING abliteration to auto-configure. Experimental Φ slower and less predictable than advanced.
- **surgical** Φ SAE features + neuron masking + head surgery + per-expert. Very slow (~1-2 hrs). Best for reasoning models.
- **optimized** Φ Bayesian hyperparameter search (Optuna TPE). Longest runtime but finds optimal parameters.
- **inverted** Φ Flips the refusal direction. Model becomes actively willing.
- **nuclear** Φ Maximum force combo for stubborn MoE models. Expert-granular.

### Direction Extraction Methods (--direction-method flag)
- **diff_means** (default) Φ Simple difference-in-means between refused/complied activations. Robust.
- **svd** Φ Multi-direction SVD extraction. Better for complex alignment.
- **leace** Φ LEACE (Linear Erasure via Closed-form Estimation). Optimal linear erasure.

### 4 Python-API-Only Methods
(NOT available via CLI Φ require Python import, which violates AGPL boundary. Mention to user only if they explicitly want to use OBLITERATUS as a library in their own AGPL project.)
...
### Standard usage
```bash
# Default method (advanced) Φ recommended for most models
obliteratus obliterate <model_name> --method advanced --output-dir ./abliterated-models

# With 4-bit quantization (saves VRAM)
obliteratus obliterate <model_name> --method advanced --quantization 4bit --output-dir ./abliterated-models

# Large models (70B+) Φ conservative defaults
...
```bash
# Interactive guided mode (hardware Φ model Φ preset)
...
```python
# Python API only Φ for user's own projects
...
- **Embedding Ablation** Φ Target embedding layer components
- **FFN Ablation** Φ Feed-forward network block removal
- **Head Pruning** Φ Attention head pruning
- **Layer Removal** Φ Full layer removal
...
- **CUDA** Φ Full support (NVIDIA GPUs)
- **Apple Silicon (MLX)** Φ Supported via MLX backend
- **CPU** Φ Supported for tiny models (< 1B params)

## YAML Config Templates

Load templates for reproducible runs via `skill_view`:
- `templates/abliteration-config.yaml` Φ Standard single-model config
- `templates/analysis-study.yaml` Φ Pre-abliteration analysis study
- `templates/batch-abliteration.yaml` Φ Multi-model batch processing

## Telemetry

OBLITERATUS can optionally contribute anonymized run data to a global research dataset.
Enable with `--contribute` flag. No personal data is collected Φ only model name, method, metrics.

## Common Pitfalls

1. **Don't use `informed` as default** Φ it's experimental and slower. Use `advanced` for reliable results.
2. **Models under ~1B respond poorly to abliteration** Φ their refusal behaviors are shallow and fragmented, making clean direction extraction difficult. Expect partial results (20-40% remaining refusal). Models 3B+ have cleaner refusal directions and respond much better (often 0% refusal with `advanced`).
3. **`aggressive` can make things worse** Φ on small models it can damage coherence and actually increase refusal rate. Only use it if `advanced` leaves > 10% refusals on a 3B+ model.
4. **Always check perplexity** Φ if it spikes > 15%, the model is damaged. Reduce aggressiveness.
5. **MoE models need special handling** Φ use `nuclear` method for Mixtral, DeepSeek-MoE, etc.
6. **Quantized models can't be re-quantized** Φ abliterate the full-precision model, then quantize the output.
7. **VRAM estimation is approximate** Φ 4-bit quant helps but peak usage can spike during extraction.
8. **Reasoning models are sensitive** Φ use `surgical` for R1 distills to preserve chain-of-thought.
9. **Check `obliteratus recommend`** Φ telemetry data may have better parameters than defaults.
10. **AGPL license** Φ never `import obliteratus` in MIT/Apache projects. CLI invocation only.
11. **Large models (70B+)** Φ always use `--large-model` flag for conservative defaults.
12. **Spectral certification RED is common** Φ the spectral check often flags "incomplete" even when practical refusal rate is 0%. Check actual refusal rate rather than relying on spectral certification alone.

## Complementary Skills

- **vllm** Φ Serve abliterated models with high throughput
- **gguf** Φ Convert abliterated models to GGUF for llama.cpp
- **huggingface-tokenizers** Φ Work with model tokenizers
