# OBLITERATUS Methods Φ Detailed Guide
...
Abliteration identifies a "refusal direction" Φ a vector in the model's activation space that
corresponds to refusal behavior Φ and projects it out of the weight matrices.
...
| LEACE | `--direction-method leace` | Linear Erasure via Closed-form Estimation Φ mathematically optimal | Maximum precision, research |
...
### advanced (DEFAULT Φ RECOMMENDED)
...
- **Risk:** Variable Φ results depend on analysis quality
...
- **How it works:** Uses SAE (Sparse Autoencoder) features + individual neuron masking + attention head surgery + per-expert decomposition (for MoE). CoT-aware Φ identifies and protects reasoning-critical directions before projecting.

### optimized
- **Speed:** Very slow (hours Φ runs many trials)
...
Do you need the absolute best quality and have time?
  Φ YES: optimized
  Φ NO: advanced (recommended default)

Did advanced leave > 10% refusals?
  Φ YES: aggressive
  Φ Still refusing: nuclear
```
