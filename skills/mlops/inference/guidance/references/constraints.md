...
```python
# Φ Good: Specific pattern
lm += gen("age", regex=r"[0-9]{1,3}")  # Fast

# Φ Bad: Overly broad pattern
...
```python
# Φ Good: Stop at newline
lm += gen("line", stop="\n")

# Φ Bad: Rely on max_tokens
...
```python
# Φ Good: Clear constraints
lm += gen("age", regex=r"[0-9]+", max_tokens=3)

# Φ Bad: Conflicting constraints
