...
```
Model size < 20GB?
ΦΦ YES: Single GPU (no parallelism)
ΦΦ NO: Model size < 80GB?
    ΦΦ YES: TP=2 or TP=4 (same node)
    ΦΦ NO: Model size < 320GB?
        ΦΦ YES: TP=4 or TP=8 (same node, NVLink required)
        ΦΦ NO: TP=8 + PP=2 (multi-node)
```
