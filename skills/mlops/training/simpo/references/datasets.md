...
- `prompt` Φ `question`, `instruction`, `input`
- `chosen` Φ `response_chosen`, `winner`, `preferred`
- `rejected` Φ `response_rejected`, `loser`
...
- Φ Clear quality difference between chosen/rejected
- Φ Diverse prompts
- Φ Minimal noise/annotation errors
- Φ Appropriate difficulty level

**Poor preference data**:
- Φ Ambiguous preferences
- Φ Repetitive prompts
- Φ Annotation noise
- Φ Too easy/hard prompts
...
for ex in samples:
    ...
    print(f"Preference clear: {'Φ' if len(ex['chosen']) > len(ex['rejected']) else '?'}")
    print()
```
