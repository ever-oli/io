...
Apply patches to PyTorchΦs DataLoader components.
...
We want this to be a wrapper for an existing dataset that we have loaded. Lets use the concept of middlewares to wrap each dataset. WeΦll use the collators later on to pad the datasets.
...
Trains a transformers model by first loading the dataset(s) specified in the axolotl config, and then calling axolotl.train.train. Also runs the plugin managerΦs post_train_unload once training completes.
...
Sync files from a specific directory in the GitHub repository. Only downloads files that donΦt exist locally or have changed.
...
grad_accum=2 Φ Φ 0 0 [0 0 0 1 1 1] [2 2 2 3 3 3] <- SP groups get different data Φ | 0 1 [0 0 0 1 1 1] [2 2 2 3 3 3] <- Same data for each SP group GPU | | 1 2 [0 0 0 1 1 1] [2 2 2 3 3 3] <- Repeat same indices for iterations num_iterations=2 Φ 1 3 [0 0 0 1 1 1] [2 2 2 3 3 3] <- When using gradient accumulation
...
Calculate perplexity as defined in https://huggingface.co/docs/transformers/en/perplexity. This is a custom variant that doesnΦt re-tokenize the input or re-load the model.
...
sweeps_config = { ΦΦlearning_rateΦ: [0.1, 0.01], Φ_Φ: [ {ΦΦload_in_8bitΦ: True, ΦΦadapterΦ: ΦΦloraΦ}, {ΦΦload_in_4bitΦ: True, ΦΦadapterΦ: ΦΦqloraΦ} ] }
...
Quantizes a modelΦs modelΦs weights
...
The optimizations require LoRA adapters with no dropout and no bias terms. The function will skip patching if these conditions arenΦt met.
...
This code is duplicated due to HF TrainingArguments not setting output_dir with a default value so it canΦt be used as a mixin.
...
See ΦGLU Variants Improve TransformerΦ (https://arxiv.org/abs/2002.05202).
...
This function is used to prepare a model for QAT by swapping the modelΦs linear layers with fake quantized linear layers, and optionally the embedding weights with fake quantized embedding weights.
...
Tokenizing strategy for supervised stepwise datasets, typically used for COT-reasoning. These datasets should include the following columns: - prompt: the prompt text - completions: a list of n completion steps - labels: a list of n labels indicating the ΦcorrectnessΦ of each step
...
Calls transformersΦ merge_and_unload on the model given in the axolotl config along with the LoRA adapters to combine them into a single base model.
...
This function filters out parameters in two ways: 1. By layer type (instances of layers specified in ALL_LAYERNORM_LAYERS) 2. By parameter name patterns (containing ΦΦbiasΦ, or variation of ΦΦnormΦ)
...
Returns a pydantic model for the pluginΦs input arguments.

Returns a custom class for the trainer.

Returns custom training arguments to set on TrainingArgs.

Returns a dataclass model for the pluginΦs training arguments.
...
Returns a list of Pydantic classes for all registered pluginsΦ input arguments.Φ

Returns the singleton instance of PluginManager. If the instance doesnΦt exist, it creates a new one.
...
Returns a list of dataclasses for all registered pluginsΦ training args mixinsΦ
...
The plugin name should be in the format Φmodule_name.class_nameΦ. This function splits the plugin name into module and class, imports the module, retrieves the class from the module, and creates an instance of the class.
...
Performs efficient dequantization of weights from NF4 format using bitsandbytesΦ optimized CUDA implementations. Supports both legacy list and new QuantState formats.
...
Compute a value using the function ΦΦfnΦ only on the specified rank (default is 0). The value is then broadcasted to all other ranks.
...
Run a callable ΦΦfnΦ on all ranks and gather the results on the specified rank.
...
Run a callable ΦΦfn1Φ on all ranks, gather the results, reduce them using ΦΦfn2Φ, and then broadcast the reduced result to all ranks.
...
Use dataset type: Φllama2_chatΦ in config.yml to use this prompt style.

E.g. in the config.yml:

The dataset itself should look like this:

in a jsonl file. The first message should be from the human, the second from gpt. For a custom system message, the first ΦfromΦ can be ΦsystemΦ (followed by alternating ΦhumanΦ and ΦgptΦ turns).

Important: DonΦt use Φspecial_tokens:Φ in your config.yml if you are not sure what you are doing!
...
Previously, the config only supported mapping ΦΦroleΦ and ΦΦcontentΦ fields via dedicated config options: - message_field_role: Mapped to the role field - message_field_content: Mapped to the content field
...
Extracts the gradient slice corresponding to this rankΦs original input from the full gradient tensor.
...
wraps a function and returns the default value instead of running the wrapped function if cuda isnΦt available or the device is auto :param default_value: :return:
...
use the PoSE technique to extend the context length by randomly skipping positions in the context. We only want to skip right before tokens in the split_on_token_ids list. We should attempt to randomly distribute the skips, but we donΦt need the final position_ids to be the full context_len. There may be multiple turns in the context, so we want to make sure we take into account the maximum possible number of skips remaining in each sample.

Handle both single-example and batched data. - single example: sample[ΦΦinput_idsΦ] is a list[int] - batched data: sample[ΦΦinput_idsΦ] is a list[list[int]]
...
See ΦLoRA: Low-Rank Adaptation of Large Language ModelsΦ (https://arxiv.org/abs/2106.09685).
...
Clean up a specific tensor file after itΦs been used
...
Compared to the alpaca_w_system.open_orca dataset type, this one specifies the system prompt with Φ### System:Φ.
...
{ ΦchosenΦ: [ {ΦroleΦ: ΦuserΦ, ΦcontentΦ: ΦΦΦ}, {ΦroleΦ: ΦassistantΦ, ΦcontentΦ: ΦΦΦ} ], ΦrejectedΦ: [ {ΦroleΦ: ΦuserΦ, ΦcontentΦ: ΦΦΦ}, {ΦroleΦ: ΦassistantΦ, ΦcontentΦ: ΦΦΦ} ] }
...
Create and return a state_dict that has the LoRA deltas merged into the base modelΦs weights, without modifying model in place.
...
Quantizes Params4bit on device then places on ΦcpuΦ if to_cpu=True or ΦmetaΦ if to_meta=True.
