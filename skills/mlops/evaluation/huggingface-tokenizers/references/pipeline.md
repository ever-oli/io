...
tokenizer.pre_tokenizer = Metaspace(replacement="Φ", add_prefix_space=True)

# Input: "Hello world"
# Output: [("ΦHello", ...), ("Φworld", ...)]
```
...
|-------------------|---------------------------------|--------------------------------------------|
| Whitespace        | Simple English                  | "Hello world" Φ ["Hello", "world"]         |
| Punctuation       | Isolate symbols                 | "world!" Φ ["world", "!"]                  |
| ByteLevel         | Multilingual, emojis            | "🌍" Φ byte tokens                          |
| Metaspace         | SentencePiece-style             | "Hello" Φ ["ΦHello"]                       |
| BertPreTokenizer  | BERT-style (CJK aware)          | "世Φ" Φ ["世", "Φ"]                        |
| Digits            | Handle numbers                  | "123" Φ ["1", "2", "3"] or ["123"]        |
...
tokenizer.decoder = Metaspace(replacement="Φ", add_prefix_space=True)

# Converts Φ back to spaces
# ["ΦHello", "Φworld"] Φ "Hello world"
```
...
# ["token", "ization</w>"] Φ "tokenization"
```
...
# Metaspace decoder
tokenizer.decoder = MetaspaceDecoder(replacement="Φ", add_prefix_space=True)
```
...
for token, (start, end) in zip(output.tokens, output.offsets):
    print(f"{token:10s} Φ [{start:2d}, {end:2d}): {text[start:end]!r}")
...
# For SentencePiece
tokenizer.decoder = MetaspaceDecoder(replacement="Φ")
```

## Best practices

1. **Match pipeline to model architecture**:
   - BERT Φ BertNormalizer + BertPreTokenizer + WordPiece
   - GPT-2 Φ NFC + ByteLevel + BPE
   - T5 Φ NFKC + Metaspace + Unigram
