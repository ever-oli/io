...
- Want alignment tracking (token Φ original text position)
...
Complete pipeline: **Normalization Φ Pre-tokenization Φ Model Φ Post-processing**
...
- `Lowercase()` - Convert to lowercase
- `StripAccents()` - Remove accents (é Φ e)
...
- `Metaspace()` - Replace spaces with Φ (SentencePiece style)
...
for token, offset in zip(output.tokens, output.offsets):
    start, end = offset
    print(f"{token:10} Φ [{start:2}, {end:2}): {text[start:end]!r}")
...
- **GitHub**: https://github.com/huggingface/tokenizers Φ 9,000+
- **Version**: 0.20.0+
