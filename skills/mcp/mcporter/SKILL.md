...
# Machine-readable output (recommended for IO)
...
- Ad-hoc servers (HTTP URL or `--stdio` command) work without any config Φ useful for one-off calls
- OAuth auth may require interactive browser flow Φ use `terminal(command="mcporter auth <server>", pty=true)` if needed
