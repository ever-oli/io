...
- Vertical rain/matrix effects are naturally enhanced Φ longer column travel
...
```python
# Build per-cell color from effect fields (cheap Φ rows*cols, not VH*VW)
...
# Render background Φ single matrix multiply, no per-cell loop
canvas_bg = g.render_bg(color_field, PAL_DENSE)
```

The texture init loop runs once and is cached per palette. Per-frame cost is one fancy-index lookup + one broadcast multiply Φ orders of magnitude faster than the per-cell bitmap blit loop in `render()` for dense backgrounds.
...
def render_text_layer(grid, rows_data, font):
    ...
    Args:
        ...
        rows_data: list of (row_index, text_string, rgb_tuple) Φ one per row
        font: PIL ImageFont instance (grid.font)

    Returns:
        uint8 array (VH, VW, 3) Φ canvas with rendered text
    """
...
This is purely a rendering optimization Φ same visual output, fewer draw calls. The grid's `render()` method is still needed for sparse character fields where characters are placed individually based on value fields.
...
def cleanup_render_artifacts(segments_dir="segments", keep_final=True):
    ...
    # 4. Feature cache (optional Φ useful to keep for re-renders)
...
if os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
    ...
    print("WARNING: final output missing or empty Φ skipping cleanup")
```

### Temp File Best Practices

- Use `tempfile.mkdtemp()` for segment directories Φ avoids polluting the project dir
...
- Feature caches (`.npz`) are cheap to store and expensive to recompute Φ default to keeping them
