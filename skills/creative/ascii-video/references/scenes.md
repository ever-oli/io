...
- A **concept** Φ what is happening visually? Not "plasma + rings" but "emergence from void" or "crystallization"
- An **arc** Φ how does it change over its duration? Build, decay, transform, reveal?
- A **role** Φ how does it serve the larger video narrative? Opening tension, peak energy, resolution?
...
|-------|------|-----------|---------|
| **Background** | xs or sm (dense) | 0.1Φ0.25 | Atmosphere, texture. Never competes with content. |
| **Content** | md (balanced) | 0.4Φ0.8 | The main visual idea. Carries the scene's concept. |
| **Accent** | lg or sm (sparse) | 0.5Φ1.0 (sparse coverage) | Highlights, punctuation, sparse bright points. |
...
Parameters should *go somewhere* over the scene's duration Φ not oscillate aimlessly with `sin(t * N)`.

**Bad:** `twist = 3.0 + 2.0 * math.sin(t * 0.6)` Φ wobbles back and forth, feels aimless.

**Good:** `twist = 2.0 + progress * 5.0` Φ starts gentle, ends intense. The scene *builds*.

Use `progress = min(local / duration, 1.0)` (0Φ1 over the scene) to drive directional change:
...
|--------------|-----------|-----|
| Emergence | Ring radius | 0 Φ max (ease-out) |
| Shatter | Voronoi cell count | 8 Φ 38 (linear) |
| Descent | Tunnel speed | 2.0 Φ 10.0 (linear) |
| Mandala | Shape complexity | ring Φ +polygon Φ +star Φ +rosette (step reveals) |
| Crescendo | Layer count | 1 Φ 7 (staggered entry) |
| Entropy | Geometry visibility | 1.0 Φ 0.0 (consumed) |

## Scene Concepts

Each scene should be built around a *visual idea*, not an effect name.

**Bad:** "fx_plasma_cascade" Φ named after the effect. No concept.
**Good:** "fx_emergence" Φ a point of light expands into a field. The name tells you *what happens*.
...
| Entropy | Decay, dissolution | none | Clean, no persistence Φ things disappear |
...
```python
collision_phase = abs(progress - 0.5) * 2  # 1Φ0Φ1 (0 at collision)
...
Voronoi with cell count increasing over time Φ visual shattering:

```python
n_pts = int(8 + progress * 30)  # 8 cells Φ 38 cells
...
# Render geometry first, organic on top Φ organic consumes geometry
```
...
- **Vary mood between adjacent scenes** Φ don't put two calm scenes next to each other
- **Randomize order** rather than grouping by type Φ prevents "effect demo" feel
- **End on the strongest scene** Φ crescendo or something with a clear payoff
- **Open with energy** Φ grab attention in the first 2 seconds
...
def fx_scene_name(r, f, t, S) -> canvas:
    """
    Args:
        r: Renderer instance Φ access multiple grids via r.get_grid("sm")
        f: dict of audio/video features, all values normalized to [0, 1]
        t: time in seconds Φ local to scene (0.0 at scene start)
        S: dict for persistent state (particles, rain columns, etc.)

    Returns:
        canvas: numpy uint8 array, shape (VH, VW, 3) Φ full pixel frame
    """
```
...
```python
progress = min(t / scene_duration, 1.0)  # 0Φ1 over the scene
```
...
def fx_matrix_layered(r, f, t, S):
    """Matrix rain blended with tunnel Φ two grids, screen blend."""
...
1. **Choose 2-3 grid sizes** Φ different scales create interference
2. **Choose different value fields** per layer Φ don't use the same effect on every grid
3. **Choose different hue fields** per layer Φ or at minimum different hue offsets
4. **Choose different palettes** per layer Φ mixing PAL_RUNE with PAL_BLOCKS looks different from PAL_RUNE with PAL_DENSE
5. **Choose a blend mode** that matches the energy Φ screen for bright, difference for psychedelic, exclusion for subtle
6. **Add conditional effects** on beat Φ kaleidoscope, mirror, glitch
7. **Configure feedback** for trailing/recursive looks Φ or None for clean cuts
...
### Minimal Φ Single Grid, Single Effect
...
def fx_coral(r, f, t, S):
    """Gray-Scott reaction-diffusion Φ coral branching pattern.
...
### Complex Φ Three Grids + Conditional + Feedback
...
def fx_masked_life(r, f, t, S):
    ...
    # Animated iris mask Φ opens over first 5 seconds of scene
    ...
    ch_p, co_p = update_flow_particles(S, g_sm, f, rd_field,
        n=300, speed=0.8, char_set=list("·ΦΦΦΦ°"))
...
### Specialized Φ Unique State Patterns
...
def fx_boid_swarm(r, f, t, S):
    ...
    ch_b, co_b = update_boids(S, g, f, n_boids=150, perception=6.0,
                              max_speed=1.5, char_set=list("ΦΦΦΦΦΦ"))
...
def fx_portrait_rain_quote(r, f, t, S):
    ...
    # Matrix rain Φ long trails benefit from portrait's extra rows
    ...
    # Quote text Φ portrait layout: short lines, many of them
