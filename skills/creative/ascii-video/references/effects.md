...
def update_flow_particles(S, g, f, flow_field, n=500, speed=1.0,
    ...
    flow_field: float32 (rows, cols) Φ the field particles follow.
                Particles flow from low to high values (uphill) or along
                the gradient direction.
    """
    if char_set is None:
        char_set = list("·ΦΦΦΦ°Φ")
    if "fp_x" not in S:
...
# Usage:
stack = LayerStack()
stack.add(bg_effect)           # base Φ fills screen
...
### Effect Nodes Φ Uniform Interface
...
def vf_tunnel(g, f, t, S, speed=3.0, complexity=6):
    """Tunnel depth effect Φ infinite zoom feeling."""
...
def vf_vortex(g, f, t, S, twist=3.0):
    """Twisting radial pattern Φ distance modulates angle."""
...
def vf_noise_static(g, f, t, S, density=0.4):
    """Random noise Φ different each frame. Non-deterministic."""
...
These produce qualitatively different textures from sine-based fields Φ organic, non-repeating, without visible axis alignment. They're the foundation of high-end generative art.
...
def vf_fbm(g, f, t, S, octaves=5, freq=0.06, lacunarity=2.0, gain=0.5,
           speed=0.2, bri=0.8):
    """Fractal Brownian Motion Φ octaved noise with lacunarity/gain control.
...
def vf_domain_warp(g, f, t, S, base_fn=None, warp_fn=None,
    ...
    """Domain warping Φ feed one noise field's output as coordinate offsets
...
def vf_voronoi(g, f, t, S, n_cells=20, speed=0.3, edge_width=1.5,
    ...
    mode: "distance" (bright at center, dark at edges),
          ...
          "cell_id" (flat color per cell Φ use with discrete palette)
...
def vf_strange_attractor(g, f, t, S, attractor="clifford",
    ...
    params: (a, b, c, d) floats Φ each attractor has different sweet spots.
...
def sdf_glow(dist, falloff=0.05):
    """Render SDF as glowing outline Φ bright at boundary, fading both directions."""
...
def sdf_star(g, cx=0.5, cy=0.5, n_points=5, outer_r=0.25, inner_r=0.12):
    """Star polygon SDF Φ n-pointed star."""
...
def sdf_union(d1, d2):
    """Boolean union Φ shape is wherever either SDF is inside."""
    return np.minimum(d1, d2)

def sdf_intersect(d1, d2):
    """Boolean intersection Φ shape is where both SDFs overlap."""
    return np.maximum(d1, d2)

def sdf_subtract(d1, d2):
    """Boolean subtraction Φ d1 minus d2."""
    return np.maximum(d1, -d2)

def sdf_smooth_union(d1, d2, k=0.1):
    """Smooth minimum (polynomial) Φ blends shapes with rounded join.
    k: smoothing radius. Higher = more rounding."""
    h = np.clip(0.5 + 0.5 * (d2 - d1) / k, 0, 1)
    return d2 * (1 - h) + d1 * h - k * h * (1 - h)

def sdf_smooth_subtract(d1, d2, k=0.1):
    """Smooth subtraction Φ d1 minus d2 with rounded edge."""
...
def sdf_repeat(g, sdf_fn, spacing_x=0.25, spacing_y=0.25, **sdf_kwargs):
    ...
    # This is a simplified approach Φ build a temporary namespace
    class ModGrid:
        pass
    mg = ModGrid()
...
def hf_angle(offset=0.0):
    """Hue mapped to angle from center Φ rainbow wheel."""
    def fn(g, f, t, S):
...
def hf_audio_cent():
    """Hue follows spectral centroid Φ timbral color shifting."""
    def fn(g, f, t, S):
...
def hf_gradient_h(start=0.0, end=1.0):
    ...
    def fn(g, f, t, S):
        h = np.broadcast_to(
            ...
            (g.rows, g.cols)
        ).copy()  # .copy() is CRITICAL Φ see troubleshooting.md
        return h % 1.0
    return fn
...
def hf_plasma(speed=0.3):
    """Plasma-style hue field Φ organic color variation."""
    def fn(g, f, t, S):
...
def uv_rotate(g, angle):
    ...
    Returns (rotated_cc, rotated_rr) arrays Φ use in place of g.cc, g.rr."""
...
def ease_elastic(t):
    """Elastic ease-out Φ overshoots then settles."""
    return np.where(t == 0, 0, np.where(t == 1, 1,
        2**(-10*t) * np.sin((t*10 - 0.75) * (2*np.pi) / 3) + 1))
def ease_bounce(t):
    """Bounce ease-out Φ bounces at the end."""
...
3D noise sampled at `(x, y, t)` Φ patterns evolve smoothly in time without per-frame discontinuities:
...
def vf_temporal_noise(g, f, t, S, freq=0.06, t_freq=0.3, octaves=4,
    ...
    this morphs the pattern in-place Φ cells brighten and dim without
...
### Full Scene Example (v2 Φ Canvas Return)
...
## Combining Effects Φ Creative Guide
...
- **Content** (md): the main visual Φ rings, voronoi, spirals, tunnel. Full brightness.
- **Accent** (lg or xl): sparse highlights Φ particles, text stencil, glow pulse. Screen-blended on top.
