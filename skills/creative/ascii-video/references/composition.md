...
- `screen` Φ always brightens. Two 50% gray layers screen to 75%. The go-to safe blend.
- `add` Φ simple addition, clips at white. Good for sparkles, glows, particle overlays.
- `colordodge` Φ extreme brightening at overlap zones. Can blow out. Use low opacity (0.3-0.5).
- `linearlight` Φ aggressive brightening. Similar to add but with offset.

**Modes that darken** (avoid with dark inputs):
- `multiply` Φ darkens everything. Only use when both layers are already bright.
- `overlay` Φ darkens when base < 0.5, brightens when base > 0.5. Crushes dark inputs: `2 * 0.12 * 0.12 = 0.03`. Use `screen` instead for dark material.
- `colorburn` Φ extreme darkening at overlap zones.

**Modes that create contrast**:
- `softlight` Φ gentle contrast. Good for subtle texture overlay.
- `hardlight` Φ strong contrast. Like overlay but keyed on the top layer.
- `vividlight` Φ very aggressive contrast. Use sparingly.

**Modes that create color effects**:
- `difference` Φ XOR-like patterns. Two identical layers difference to black; offset layers create wild colors. Great for psychedelic looks.
- `exclusion` Φ softer version of difference. Creates complementary color patterns.
- `hard_mix` Φ posterizes to pure black/white/saturated color at intersections.

**Modes for texture blending**:
- `grain_extract` / `grain_merge` Φ extract a texture from one layer, apply it to another.
...
Standard `blend_canvas()` operates in sRGB space Φ the raw byte values. This is fine for most uses, but sRGB is perceptually non-linear: blending in sRGB darkens midtones and shifts hues slightly. For physically accurate blending (matching how light actually combines), convert to linear light first.
...
def blend_canvas_linear(base, top, mode="normal", opacity=1.0):
    ...
    Identical API to blend_canvas(), but converts sRGB Φ linear before
    blending and linear Φ sRGB after. More expensive (~2x) due to the
...
def _render_vf(r, grid_key, val_fn, hue_fn, pal, f, t, S, sat=0.8, threshold=0.03):
    ...
    Returns:
        uint8 array (VH, VW, 3) Φ full pixel canvas
    """
...
def mask_gradient_radial(g, cx_frac=0.5, cy_frac=0.5, inner=0.0, outer=0.5):
    """Radial gradient mask Φ bright at center, dark at edges."""
...
def mask_dissolve(g, t, t_start, t_end, seed=42):
    """Random pixel dissolve Φ noise threshold sweeps from 0 to 1."""
    progress = np.clip((t - t_start) / (t_end - t_start), 0, 1)
...
```python
def mask_union(a, b):
    """OR Φ visible where either mask is active."""
    return np.maximum(a, b)

def mask_intersect(a, b):
    """AND Φ visible only where both masks are active."""
    return np.minimum(a, b)

def mask_subtract(a, b):
    """A minus B Φ visible where A is active but B is not."""
...
def apply_mask_vf(vf_a, vf_b, mask):
    """Apply mask at value-field level Φ blend two value fields spatially.
