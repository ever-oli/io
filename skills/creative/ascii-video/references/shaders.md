...
The shader pipeline turns raw ASCII renders into cinematic output. The system is designed for **composability** Φ every shader, blend mode, and feedback transform is an independent building block. Combining them creates infinite visual variety from a small set of primitives.
...
Composable shader pipeline. Build chains of named shaders with parameters. Order matters Φ shaders are applied sequentially to the canvas.
...
### `_apply_shader_step()` Φ Full Dispatch Function

Routes shader names to implementations. Some shaders have **audio-reactive scaling** Φ the dispatch function reads `f["bdecay"]` and `f["rms"]` to modulate parameters on the beat.
...
def _apply_shader_step(canvas, name, kwargs, f, t):
    ...
    Returns:
        canvas: uint8 (H,W,3) Φ processed
    """
    ...
    # --- Glitch / Data ---
        ...
        return canvas  # unknown shader Φ passthrough
```
...
|--------|------------|--------|
| `chromatic` | `bdecay` | `amt * (0.4 + bdecay * 0.8)` Φ aberration kicks on beats |
| `color_wobble` | `rms` | `amt * (0.5 + rms * 0.8)` Φ wobble intensity follows energy |
| `grain` | `rms` | `amt * (0.5 + rms * 0.8)` Φ grain rougher in loud sections |
...
| `mirror_h` | Φ | Horizontal mirror |
| `mirror_v` | Φ | Vertical mirror |
| `mirror_quad` | Φ | 4-fold mirror |
| `mirror_diag` | Φ | Diagonal mirror |
...
Cache the coordinate remap Φ it never changes per frame:
```python
_crt_cache = {}
...
```python
def sh_mirror_h(c):
    """Horizontal mirror Φ left half reflected to right."""
    w = c.shape[1]; c[:, w//2:] = c[:, :w//2][:, ::-1]; return c

def sh_mirror_v(c):
    """Vertical mirror Φ top half reflected to bottom."""
    h = c.shape[0]; c[h//2:, :] = c[:h//2, :][::-1, :]; return c

def sh_mirror_quad(c):
    """4-fold mirror Φ top-left quadrant reflected to all four."""
...
def sh_mirror_diag(c):
    """Diagonal mirror Φ top-left triangle reflected."""
    h, w = c.shape[:2]
    for y in range(h):
        x_cut = int(w * y / h)
...
def sh_rgb_split_radial(c, strength=5):
    """Chromatic aberration radiating from center Φ stronger at edges."""
    h, w = c.shape[:2]; cy, cx = h//2, w//2
    Y = np.arange(h, dtype=np.float32)[:, None]
    X = np.arange(w, dtype=np.float32)[None, :]
...
def sh_solarize(c, threshold=128):
    """Invert pixels above threshold Φ classic darkroom effect."""
...
def sh_radial_blur(c, strength=0.03, center=None):
    """Zoom blur from center Φ motion blur radiating outward."""
    h, w = c.shape[:2]
...
def sh_halftone(c, dot_size=6):
    """Halftone dot pattern overlay Φ circular dots sized by local brightness."""
    h, w = c.shape[:2]
    gray = c.astype(np.float32).mean(axis=2) / 255.0
    out = np.zeros_like(c)
...
def sh_block_glitch(c, n_blocks=8, max_size=40):
    """Random rectangular block displacement Φ copy blocks to random positions."""
...
def sh_data_bend(c, offset=1000, chunk=500):
    """Treat raw pixel bytes as data, copy a chunk to another offset Φ datamosh artifacts."""
    flat = c.flatten().copy()
    n = len(flat)
...
def create_rgba_canvas(H, W):
    """Transparent canvas Φ alpha channel starts at 0 (fully transparent)."""
...
def render_char_rgba(canvas, row, col, char_img, color_rgb, alpha=255):
    ...
    Alpha comes from the glyph mask Φ background stays transparent."""
    r, g, b = color_rgb
...
def render_live(scene_fn, r, fps=24, duration=None):
    ...
    try:
        while True:
            ...
            # Render scene Φ for terminal, use a small grid
            g = r.get_grid("sm")
            # Option A: v2 scene Φ extract chars/colors from canvas (reverse render)
            ...
            # (bypassing the pixel canvas Φ terminal uses character cells)
