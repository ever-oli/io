...
Hue field generators often return arrays that are broadcast views Φ they have shape `(1, cols)` or `(rows, 1)` that numpy broadcasts to `(rows, cols)`. These views are **read-only**. If any downstream code tries to modify them in-place (e.g., `h %= 1.0`), numpy raises:
...
`multiply(a, b) = a * b`. Since both operands are [0,1], the result is always <= min(a,b). Never use multiply as a feedback blend mode Φ the frame goes black within a few frames.
...
**Fix**: Define all scene functions at module top level. Lambdas used inside `_render_vf()` as val_fn/hue_fn are fine because they execute within the worker process Φ they're not pickled across process boundaries.
...
- `np.random` state is NOT seeded by `random.seed()` Φ they use separate RNGs
...
This fails because:
- Dark scenes (mean 8): `8 * 2.0 = 16` Φ still dark
- Bright scenes (mean 130): `130 * 2.0 = 255` Φ clipped, lost detail
...
```python
# DEADLOCK Φ stderr buffer fills at 64KB, blocks ffmpeg, blocks your writes
...
1. Check grid count Φ each extra grid adds ~100-150ms/frame for init
2. Check particle count Φ cap at quality-appropriate limits
3. Check shader count Φ each shader adds 2-25ms
...
The v2 scene protocol passes `S` (the state dict) as an explicit parameter. But `S` IS `r.S` Φ they're the same object. Both work:
...
```python
# WRONG Φ particles reset every frame
S["px"] = []
for _ in range(100):
    S["px"].append(random.random())

# RIGHT Φ only initialize once, update each frame
if "px" not in S:
    S["px"] = []
...
```python
# WRONG Φ vf_plasma() * 1.5 can exceed 1.0
val = vf_plasma(g, f, t, S) * 1.5

# RIGHT Φ clip after scaling
...
- Dense animated backgrounds Φ never flat black, always fill the grid
- Vignette minimum clamped to 0.15 (not 0.12)
- Bloom threshold 130 (not 170) so more pixels contribute to glow
- Use `screen` blend mode (not `overlay`) for dark ASCII layers Φ overlay squares dark values: `2 * 0.12 * 0.12 = 0.03`
- FeedbackBuffer decay minimum 0.5 Φ below that, feedback disappears too fast to see
...
6. Confirm vignette strength ΦΦ 0.25
