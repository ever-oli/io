...
Built for [IO](https://github.com/ever-oli/io). Usable in any coding agent. Canonical source lives here; synced to [`ever-oli/io/skills/creative/ascii-video`](https://github.com/ever-oli/io/tree/main/skills/creative/ascii-video) via PR.
...
|--------|----------|------|
| Density ramps | ` .:-=+#@Φ` | Classic ASCII art gradient |
| Block elements | ` ΦΦΦΦΦΦΦΦ` | Chunky, digital |
| Braille | ` ΦΦΦ...Φ` | Fine-grained pointillism |
| Dots | ` ΦΦΦΦΦΦΦ` | Smooth, organic |
| Stars | ` ·ΦΦΦΦΦΦΦ` | Sparkle, celestial |
| Half-fills | ` ΦΦΦΦΦΦΦΦΦΦ` | Directional fill progression |
| Crosshatch | ` ΦΦΦΦΦΦΦΦ` | Hatched density ramp |
| Math | ` ·ΦΦΦΦ°±×÷ΦΦΦΦΦΦΩ` | Scientific, abstract |
| Box drawing | ` ΦΦΦΦΦΦΦΦΦΦΦΦΦ` | Structural, circuit-like |
| Katakana | ` ·ｦｧｨｩｪｫｬｭ...` | Matrix rain |
| Greek | ` αβγδεζηθ...ω` | Classical, academic |
| Runes | ` ΦΦΦΦΦᛁᛇᛒᛖΦᛞᛟ` | Mystical, ancient |
| Alchemical | ` ΦΦΦΦΦΦΦΦΦΦΦ` | Esoteric |
| Arrows | ` ΦΦΦΦΦΦΦΦΦΦΦΦ` | Directional, kinetic |
| Music | ` ΦΦΦΦΦΦΦΦΦ` | Musical |
| Project-specific | ` .·~=ΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦ` | Themed per project |
...
Full OKLAB/OKLCH color system: sRGBΦlinearΦOKLAB conversion pipeline, perceptually uniform gradient interpolation, and color harmony generation (complementary, triadic, analogous, split-complementary, tetradic).
...
| fBM | Fractal Brownian Motion Φ octaved noise for clouds, terrain, smoke |
| Domain warp | Inigo Quilez technique Φ fBM-driven coordinate distortion for flowing organic forms |
...
| Strange attractors | Clifford, De Jong, Bedhead Φ iterated point systems binned to density fields |
...
10 easing functions (linear, quad, cubic, expo, elastic, bounce Φ in/out/in-out). Keyframe interpolation with eased transitions. Value field morphing (smooth crossfade between fields). Value field sequencing (cycle through fields with crossfade). Temporal noise (3D noise evolving smoothly in-place).
...
**Feedback buffer.** Temporal recursion Φ each frame blends with a transformed version of the previous frame. 7 spatial transforms: zoom, shrink, rotate CW/CCW, shift up/down, mirror. Optional per-frame hue shift for rainbow trails. Configurable decay, blend mode, and opacity per scene.
...
Φ Python 3.10+
Φ NumPy, Pillow, SciPy (audio modes)
Φ ffmpeg on PATH
Φ A monospace font (Menlo, Courier, Monaco, auto-detected)
Φ Optional: OpenCV, ElevenLabs API key (TTS mode)

## File structure

```
ΦΦΦ SKILL.md                 # Modes, workflow, creative direction
ΦΦΦ README.md                # This file
ΦΦΦ references/
    ΦΦΦ architecture.md      # Grid system, fonts, palettes, color, _render_vf()
    ΦΦΦ effects.md           # Value fields, hue fields, backgrounds, particles
    ΦΦΦ shaders.md           # 38 shaders, ShaderChain, tint presets, transitions
    ΦΦΦ composition.md       # Blend modes, multi-grid, tonemap, FeedbackBuffer
    ΦΦΦ scenes.md            # Scene protocol, SCENES table, render_clip(), examples
    ΦΦΦ design-patterns.md   # Layer hierarchy, directional arcs, scene concepts
    ΦΦΦ inputs.md            # Audio analysis, video sampling, text, TTS
    ΦΦΦ optimization.md      # Hardware detection, vectorized patterns, parallelism
    ΦΦΦ troubleshooting.md   # Broadcasting traps, blend pitfalls, diagnostics
```

## Projects built with this

Φ 85-second highlight reel. 15 scenes (14×5s + 15s crescendo finale), randomized order, directional parameter arcs, layer hierarchy composition. Showcases the full effect vocabulary: fBM, voronoi fragmentation, reaction-diffusion, cellular automata, dual counter-rotating spirals, wave collision, domain warping, tunnel descent, kaleidoscope symmetry, boid flocking, fire simulation, glitch corruption, and a 7-layer crescendo buildup.

Φ Audio-reactive music visualizer. 3.5 min, 8 sections with distinct effects, beat-triggered particles and glitch, cycling palettes.

Φ TTS narrated testimonial video. 23 quotes, per-quote ElevenLabs voices, background music at 15% wide stereo, per-clip re-rendering for iterative editing.
