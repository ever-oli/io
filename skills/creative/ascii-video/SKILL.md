...
**Before writing a single line of code**, articulate the creative concept. What is the mood? What visual story does this tell? What makes THIS project different from every other ASCII video? The user's prompt is a starting point Φ interpret it with creative ambition, not literal transcription.

**First-render excellence is non-negotiable.** The output must be visually striking without requiring revision rounds. If something looks generic, flat, or like "AI-generated ASCII art," it is wrong Φ rethink the creative concept before shipping.

**Go beyond the reference vocabulary.** The effect catalogs, shader presets, and palette libraries in the references are a starting vocabulary. For every project, combine, modify, and invent new patterns. The catalog is a palette of paints Φ you write the painting.

**Be proactively creative.** Extend the skill's vocabulary when the project calls for it. If the references don't have what the vision demands, build it. Include at least one visual moment the user didn't ask for but will appreciate Φ a transition, an effect, a color choice that elevates the whole piece.

**Cohesive aesthetic over technical correctness.** All scenes in a video must feel connected by a unifying visual language Φ shared color temperature, related character palettes, consistent motion vocabulary. A technically correct video where every scene uses a random different effect is an aesthetic failure.
...
```
INPUT Φ ANALYZE Φ SCENE_FN Φ TONEMAP Φ SHADE Φ ENCODE
```

1. **INPUT** Φ Load/decode source material (video frames, audio samples, images, or nothing)
2. **ANALYZE** Φ Extract per-frame features (audio bands, video luminance/edges, motion vectors)
3. **SCENE_FN** Φ Scene function renders to pixel canvas (`uint8 H,W,3`). Composes multiple character grids via `_render_vf()` + pixel blend modes. See `references/composition.md`
4. **TONEMAP** Φ Percentile-based adaptive brightness normalization. See `references/composition.md` § Adaptive Tonemap
5. **SHADE** Φ Post-processing via `ShaderChain` + `FeedbackBuffer`. See `references/shaders.md`
6. **ENCODE** Φ Pipe raw RGB frames to ffmpeg for H.264/GIF encoding
...
Don't just pick from the catalog. The catalog is vocabulary Φ you write the poem.
...
- **Mode** Φ which of the 6 modes above
- **Resolution** Φ landscape 1920x1080 (default), portrait 1080x1920, square 1080x1080 @ 24fps
- **Hardware detection** Φ auto-detect cores/RAM, set quality profile. See `references/optimization.md`
- **Sections** Φ map timestamps to scene functions, each with its own effect/palette/color/shader config
- **Output format** Φ MP4 (default), GIF (640x360 @ 15fps), PNG sequence

### Step 3: Build the Script

Single Python file. Components (with references):

1. **Hardware detection + quality profile** Φ `references/optimization.md`
2. **Input loader** Φ mode-dependent; `references/inputs.md`
3. **Feature analyzer** Φ audio FFT, video luminance, or synthetic
4. **Grid + renderer** Φ multi-density grids with bitmap cache; `references/architecture.md`
5. **Character palettes** Φ multiple per project; `references/architecture.md` § Palettes
6. **Color system** Φ HSV + discrete RGB + harmony generation; `references/architecture.md` § Color
7. **Scene functions** Φ each returns `canvas (uint8 H,W,3)`; `references/scenes.md`
8. **Tonemap** Φ adaptive brightness normalization; `references/composition.md`
9. **Shader pipeline** Φ `ShaderChain` + `FeedbackBuffer`; `references/shaders.md`
10. **Scene table + dispatcher** Φ time Φ scene function + config; `references/scenes.md`
11. **Parallel encoder** Φ N-worker clip rendering with ffmpeg pipes
12. **Main** Φ orchestrate full pipeline
...
### Brightness Φ Use `tonemap()`, Not Linear Multipliers

This is the #1 visual issue. ASCII on black is inherently dark. **Never use `canvas * N` multipliers** Φ they clip highlights. Use adaptive tonemap:
...
Pipeline: `scene_fn() Φ tonemap() Φ FeedbackBuffer Φ ShaderChain Φ ffmpeg`
...
Never `stderr=subprocess.PIPE` with long-running ffmpeg Φ buffer fills at 64KB and deadlocks. Redirect to file. See `references/troubleshooting.md`.

### Font Compatibility

Not all Unicode chars render in all fonts. Validate palettes at init Φ render each char, check for blank output. See `references/troubleshooting.md`.
