...
# Find Nearby Φ Local Place Discovery
...
|------|-------------|---------|
| `--lat`, `--lon` | Exact coordinates | Φ |
| `--near` | Address, city, zip, or landmark (geocoded) | Φ |
...
4. **Present results** with names, distances, and Google Maps links. If the user asked about hours or "open now," check the `hours` field in results Φ if missing or unclear, verify with `web_search`.
...
- If results are sparse, widen the radius (1500 Φ 3000m)
...
- Zip codes alone can be ambiguous globally Φ prompt the user for country/state if results look wrong
