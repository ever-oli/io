---
name: linear
description: Manage Linear issues, projects, and teams via the GraphQL API. Create, update, search, and organize issues. Uses API key auth (no OAuth needed). All operations via curl Φ no dependencies.
version: 1.0.0
author: IO
license: MIT
prerequisites:
...
# Linear Φ Issue & Project Management
...
2. Set `LINEAR_API_KEY` in your environment (via `io setup` or your env config)
...
Each team has its own named states (e.g., "In Progress" is type `started`). To change an issue's status, you need the `stateId` (UUID) of the target state Φ query workflow states first.
...
# Next page Φ use endCursor from previous response
curl -s -X POST https://api.linear.app/graphql \
  -H "Authorization: $LINEAR_API_KEY" \
  -H "Content-Type: application/json" \
...
- Always use `terminal` tool with `curl` for API calls Φ do NOT use `web_extract` or `browser`
- Always check the `errors` array in GraphQL responses Φ HTTP 200 can still contain errors
