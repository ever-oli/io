#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for IO.

Fully non-interactive Φ designed to be driven by the agent via terminal commands.
...
Agent workflow:
  1. Run --check. If exit 0, auth is good Φ skip setup.
...
TOKEN_PATH = IO_HOME / "google_token.json"
CLIENT_SECRET_PATH = IO_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = IO_HOME / "google_oauth_pending.json"
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to IO home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for IO")
