...
- `torchtune` may report version `0.4.0+cpu` Φ this is just package metadata, it still uses CUDA via PyTorch
...
### Performance
- RTF (Real-Time Factor) Φ 1.0 Φ a 4-minute song takes ~4 minutes to generate
- Output: MP3, 48kHz stereo, 128kbps

## Pitfalls
1. **Do NOT use bf16 for HeartCodec** Φ degrades audio quality. Use fp32 (default).
2. **Tags may be ignored** Φ known issue (#90). Lyrics tend to dominate; experiment with tag ordering.
3. **Triton not available on macOS** Φ Linux/CUDA only for GPU acceleration.
