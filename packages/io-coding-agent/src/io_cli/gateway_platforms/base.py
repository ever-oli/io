...
from ..gateway_session import SessionSource, build_session_key
from ..config import get_io_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.io/.env manually."
)
...
# Default location: {IO_HOME}/image_cache/
IMAGE_CACHE_DIR = get_io_home() / "image_cache"
...
AUDIO_CACHE_DIR = get_io_home() / "audio_cache"
...
DOCUMENT_CACHE_DIR = get_io_home() / "document_cache"
...
class BasePlatformAdapter(ABC):
    ...
    def _mark_connected(self) -> None:
        ...
        self._fatal_error_retryable = True
        try:
            from ..gateway_runtime import write_runtime_status
    ...
    async def start(self) -> None:
        ...
        if connected is False and not self.has_fatal_error:
            raise RuntimeError(f"{self.name} failed to connect")
    ...
    async def connect(self) -> bool:
        start_impl = type(self)._start
        if start_impl is BasePlatformAdapter._start:
            raise NotImplementedError(f"{type(self).__name__}.connect() is not implemented")
        await self._start()
        if not self._running and not self.has_fatal_error:
            self._mark_connected()
        return not self.has_fatal_error

    async def disconnect(self) -> None:
        stop_impl = type(self)._stop
        if stop_impl is BasePlatformAdapter._stop:
            raise NotImplementedError(f"{type(self).__name__}.disconnect() is not implemented")
        await self._stop()
        if self._running and not self.has_fatal_error:
    ...
    async def send(
        ...
        """
        send_message_impl = type(self).send_message
        if send_message_impl is BasePlatformAdapter.send_message:
    ...
    async def send_message(
        ...
        *,
        ...
        result = await self.send(chat_id, content, reply_to=thread_id, metadata=metadata)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          IO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          IO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          IO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        import random

        mode = os.getenv("IO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("IO_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("IO_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        """
        return {"id": chat_id, "name": str(chat_id), "type": "dm"}
