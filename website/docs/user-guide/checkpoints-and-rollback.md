...
IO automatically snapshots your project before **destructive operations** and lets you restore it with a single command. Checkpoints are **enabled by default** Φ there's zero cost when no file-mutating tools fire.

This safety net is powered by an internal **Checkpoint Manager** that keeps a separate shadow git repository under `~/.io/checkpoints/` Φ your real project `.git` is never touched.
...
- **Destructive terminal commands** Φ `rm`, `mv`, `sed -i`, `truncate`, `shred`, output redirects (`>`), and `git reset`/`clean`/`checkout`
...
- IO detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  ...
  - Stages and commits the current state with a short, humanΦreadable reason.
...
Checkpoints are enabled by default. Configure in `~/.io/config.yaml`:
...
When disabled, the Checkpoint Manager is a noΦop and never attempts git operations.
...
IO responds with a formatted list showing change statistics:
...
Behind the scenes, IO:

1. Verifies the target commit exists in the shadow repo.
2. Takes a **preΦrollback snapshot** of the current state so you can "undo the undo" later.
...
```text
Φ Restored to checkpoint 4270a8c5: before patch
...
To keep checkpointing safe and fast, IO applies several guardrails:

- **Git availability** Φ if `git` is not found on `PATH`, checkpoints are transparently disabled.
- **Directory scope** Φ IO skips overly broad directories (root `/`, home `$HOME`).
- **Repository size** Φ directories with more than 50,000 files are skipped to avoid slow git operations.
- **NoΦchange snapshots** Φ if there are no changes since the last snapshot, the checkpoint is skipped.
- **NonΦfatal errors** Φ all errors inside the Checkpoint Manager are logged at debug level; your tools continue to run.
...
```text
~/.io/checkpoints/
  ΦΦΦ <hash1>/   # shadow git repo for one working directory
  ΦΦΦ <hash2>/
  ΦΦΦ ...
```
...
- A `IO_WORKDIR` file pointing back to the original project root

You normally never need to touch these manually.

## Best Practices

- **Leave checkpoints enabled** Φ they're on by default and have zero cost when no files are modified.
- **Use `/rollback diff` before restoring** Φ preview what will change to pick the right checkpoint.
- **Use `/rollback` instead of `git reset`** when you want to undo agent-driven changes only.
- **Combine with Git worktrees** for maximum safety Φ keep each IO session in its own worktree/branch, with checkpoints as an extra layer.
