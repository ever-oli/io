...
IO is designed with a defense-in-depth security model. This page covers every security boundary Φ from command approval to container isolation to user authorization on messaging platforms.

## Overview

The security model has five layers:

1. **User authorization** Φ who can talk to the agent (allowlists, DM pairing)
2. **Dangerous command approval** Φ human-in-the-loop for destructive operations
3. **Container isolation** Φ Docker/Singularity/Modal sandboxing with hardened settings
4. **MCP credential filtering** Φ environment variable isolation for MCP subprocesses
5. **Context file scanning** Φ prompt injection detection in project files

## Dangerous Command Approval

Before executing any command, IO checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
```
  ΦΦ️  DANGEROUS COMMAND: recursive delete
...
- **once** Φ allow this single execution
- **session** Φ allow this pattern for the rest of the session
- **always** Φ add to permanent allowlist (saved to `config.yaml`)
- **deny** (default) Φ block the command
...
The `IO_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.io/config.yaml`:
...
:::tip
Use `io config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, IO controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.io/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.io/.env to allow open access,
...
For more flexible authorization, IO includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `io pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.io/config.yaml`:
...
| Lockout | 5 failed approval attempts Φ 1-hour lockout |
...
io pairing approve telegram ABC12DEF

# Revoke a user's access
io pairing revoke telegram 123456789

# Clear all pending codes
io pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.io/pairing/` with per-platform JSON files:
- `{platform}-pending.json` Φ pending pairing requests
- `{platform}-approved.json` Φ approved users
- `_rate_limits.json` Φ rate limit and lockout tracking

## Container Isolation

When using the `docker` terminal backend, IO applies strict security hardening to every container.
...
Container resources are configurable in `~/.io/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.io/sandboxes/docker/<task_id>/`
- **Ephemeral mode** (`container_persistent: false`): Uses tmpfs for workspace Φ everything is lost on cleanup
...
|---------|-----------|-------------------|----------|
| **local** | None Φ runs on host | Φ Yes | Development, trusted users |
| **ssh** | Remote machine | Φ Yes | Running on a separate server |
| **docker** | Container | Φ Skipped (container is boundary) | Production gateway |
| **singularity** | Container | Φ Skipped | HPC environments |
| **modal** | Cloud sandbox | Φ Skipped | Scalable cloud isolation |
| **daytona** | Cloud sandbox | Φ Skipped | Persistent cloud workspaces |
...
1. **Set explicit allowlists** Φ never use `GATEWAY_ALLOW_ALL_USERS=true` in production
2. **Use container backend** Φ set `terminal.backend: docker` in config.yaml
3. **Restrict resource limits** Φ set appropriate CPU, memory, and disk limits
4. **Store secrets securely** Φ keep API keys in `~/.io/.env` with proper file permissions
5. **Enable DM pairing** Φ use pairing codes instead of hardcoding user IDs when possible
6. **Review command allowlist** Φ periodically audit `command_allowlist` in config.yaml
7. **Set `MESSAGING_CWD`** Φ don't let the agent operate from sensitive directories
8. **Run as non-root** Φ never run the gateway as root
9. **Monitor logs** Φ check `~/.io/logs/` for unauthorized access attempts
10. **Keep updated** Φ run `io update` regularly for security patches
