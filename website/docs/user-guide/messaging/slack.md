...
sidebar_position: 4
title: "Slack"
description: "Set up IO as a Slack bot using Socket Mode"
---

# Slack Setup

Connect IO to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your IO instance doesn't need to be publicly accessible Φ it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. IO uses the modern
...
| **Connection** | WebSocket Φ no public URL required |
...
4. Enter an app name (e.g., "IO") and select your workspace
5. Click **Create App**
...
Navigate to **Features Φ OAuth & Permissions** in the sidebar. Scroll to **Scopes Φ Bot Token Scopes** and add the following:
...
Without `channels:history` and `groups:history`, the bot **will not receive messages in channels** Φ
...
1. In the sidebar, go to **Settings Φ Socket Mode**
2. Toggle **Enable Socket Mode** to ON
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `io-socket` (the name doesn't matter)
   - Add the **`connections:write`** scope
   - Click **Generate**
4. **Copy the token** Φ it starts with `xapp-`. This is your `SLACK_APP_TOKEN`

:::tip
You can always find or regenerate app-level tokens under **Settings Φ Basic Information Φ App-Level Tokens**.
:::
...
This step is critical Φ it controls what messages the bot can see.


1. In the sidebar, go to **Features Φ Event Subscriptions**
...
1. In the sidebar, go to **Settings Φ Install App**
2. Click **Install to Workspace**
3. Review the permissions and click **Allow**
4. After authorization, you'll see a **Bot User OAuth Token** starting with `xoxb-`
5. **Copy this token** Φ this is your `SLACK_BOT_TOKEN`
...
IO uses Slack **Member IDs** (not usernames or display names) for the allowlist.

To find a Member ID:

1. In Slack, click on the user's name or avatar
2. Click **View full profile**
3. Click the **Φ** (more) button
...
```bash
io gateway setup    # Select Slack when prompted
```
...
Understanding how IO behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message Φ no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@IO what time is it?`). In channels, IO replies in a thread attached to that message. |
| **Threads** | If you @mention IO inside an existing thread, it replies in that same thread. |

:::tip
In channels, always @mention the bot. Simply typing a message without mentioning it will be ignored.
This is intentional Φ it prevents the bot from responding to every message in busy channels.
:::
...
Set `SLACK_HOME_CHANNEL` to a channel ID where IO will deliver scheduled messages,
cron job results, and other proactive notifications. To find a channel ID:

1. Right-click the channel name in Slack
2. Click **View channel details**
3. Scroll to the bottom Φ the Channel ID is shown there

```bash
SLACK_HOME_CHANNEL=C01234567890
```

Make sure the bot has been **invited to the channel** (`/invite @IO`).

---

## Voice Messages

IO supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @IO` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @IO` |
...
1. Φ `message.channels` event is subscribed (for public channels)
2. Φ `message.groups` event is subscribed (for private channels)
3. Φ `app_mention` event is subscribed
4. Φ `channels:history` scope is added (for public channels)
5. Φ `groups:history` scope is added (for private channels)
6. Φ App was **reinstalled** after adding scopes/events
7. Φ Bot was **invited** to the channel (`/invite @IO`)
8. Φ You are **@mentioning** the bot in your message
...
the gateway will **deny all messages** by default as a safety measure. Never share your bot tokens Φ
treat them like passwords.
:::

- Tokens should be stored in `~/.io/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your IO config directory
- Socket Mode means no public endpoint is exposed Φ one less attack surface
