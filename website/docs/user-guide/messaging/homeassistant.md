...
description: Control your smart home with IO via Home Assistant integration.
...
IO integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:

1. **Gateway platform** Φ subscribes to real-time state changes via WebSocket and responds to events
2. **Smart home tools** Φ four LLM-callable tools for querying and controlling devices via the REST API
...
4. Click **Create Token**, give it a name like "IO"
5. Copy the token
...
IO registers four tools for smart home control:
...
**Parameters:**
- `domain` *(optional)* Φ Filter by entity domain: `light`, `switch`, `climate`, `sensor`, `binary_sensor`, `cover`, `fan`, `media_player`, etc.
- `area` *(optional)* Φ Filter by area/room name (matches against friendly names): `living room`, `kitchen`, `bedroom`, etc.
...
**Parameters:**
- `entity_id` *(required)* Φ The entity to query, e.g., `light.living_room`, `climate.thermostat`, `sensor.temperature`
...
**Parameters:**
- `domain` *(optional)* Φ Filter by domain, e.g., `light`, `climate`, `switch`
...
**Parameters:**
- `domain` *(required)* Φ Service domain: `light`, `switch`, `climate`, `cover`, `media_player`, `fan`, `scene`, `script`
- `service` *(required)* Φ Service name: `turn_on`, `turn_off`, `toggle`, `set_temperature`, `set_hvac_mode`, `open_cover`, `close_cover`, `set_volume_level`
- `entity_id` *(optional)* Φ Target entity, e.g., `light.living_room`
- `data` *(optional)* Φ Additional parameters as a JSON object

**Examples:**

```
Turn on the living room lights
Φ ha_call_service(domain="light", service="turn_on", entity_id="light.living_room")
```

```
Set the thermostat to 22 degrees in heat mode
Φ ha_call_service(domain="climate", service="set_temperature",
    entity_id="climate.thermostat", data={"temperature": 22, "hvac_mode": "heat"})
```

```
Set living room lights to blue at 50% brightness
Φ ha_call_service(domain="light", service="turn_on",
...
Configure which events the agent sees in `~/.io/gateway.json` under the Home Assistant platform's `extra` section:
...
:::tip
Start with a focused set of domains Φ `climate`, `binary_sensor`, and `alarm_control_panel` cover the most useful automations. Add more as needed. Use `ignore_entities` to suppress noisy sensors like CPU temperature or uptime counters.
:::
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "IO".

### Connection Management

- **WebSocket** with 30-second heartbeat for real-time events
- **Automatic reconnection** with backoff: 5s Φ 10s Φ 30s Φ 60s
- **REST API** for outbound notifications (separate session to avoid WebSocket conflicts)
- **Authorization** Φ HA events are always authorized (no user allowlist needed, since the `HASS_TOKEN` authenticates the connection)
...
- `shell_command` Φ arbitrary shell commands
- `command_line` Φ sensors/switches that execute commands
- `python_script` Φ scripted Python execution
- `pyscript` Φ broader scripting integration
- `hassio` Φ addon control, host shutdown/reboot
- `rest_command` Φ HTTP requests from HA server (SSRF vector)
...
1. ha_list_entities(domain="binary_sensor")
     Φ checks door/window sensors
2. ha_get_state(entity_id="alarm_control_panel.home")
     Φ checks alarm status
3. ha_list_entities(domain="lock")
     Φ checks lock states
