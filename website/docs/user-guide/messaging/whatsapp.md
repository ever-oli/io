...
sidebar_position: 5
title: "WhatsApp"
description: "Set up IO as a WhatsApp bot via the built-in Baileys bridge"
---

# WhatsApp Setup

IO connects to WhatsApp through a built-in bridge based on **Baileys**. This works by emulating a WhatsApp Web session Φ **not** through the official WhatsApp Business API. No Meta developer account or Business verification is required.

:::warning Unofficial API Φ Ban Risk
...
- **Don't send bulk/spam messages** Φ keep usage conversational
...
with third-party bridges. When this happens, IO will update the bridge dependency. If the
bot stops working after a WhatsApp update, pull the latest IO version and re-pair.
:::
...
- **Node.js v18+** and **npm** Φ the WhatsApp bridge runs as a Node.js process
...
| **Prepaid SIM** | $5Φ15 one-time | Any carrier. Activate, verify WhatsApp, then the SIM can sit in a drawer. Number must stay active (make a call every 90 days). |
| **VoIP services** | FreeΦ$5/month | TextNow, TextFree, or similar. Some VoIP numbers are blocked by WhatsApp Φ try a few if the first doesn't work. |
...
3. Run `io whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.io/whatsapp/session`. This means:

- **Sessions survive restarts** Φ you don't need to re-scan the QR code every time
- The session data includes encryption keys and device credentials
- **Do not share or commit this session directory** Φ it grants full access to the WhatsApp account
...
IO supports voice on WhatsApp:

- **Incoming:** Voice messages (`.ogg` opus) are automatically transcribed using the configured STT provider: local `faster-whisper`, Groq Whisper (`GROQ_API_KEY`), or OpenAI Whisper (`VOICE_TOOLS_OPENAI_KEY`)
- **Outgoing:** TTS responses are sent as MP3 audio file attachments
- Agent responses are prefixed with "ΦΦΦ **IO**" by default. You can customize or disable this in `config.yaml`:

```yaml
# ~/.io/config.yaml
whatsapp:
  reply_prefix: ""                          # Empty string disables the header
  # reply_prefix: "Φ *My Bot*\nΦΦΦΦΦΦ\n"  # Custom prefix (supports \n for newlines)
```
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `io whatsapp`. |
| **Session not persisting** | Check that `~/.io/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `io whatsapp` if needed. |
| **Bridge crashes or reconnect loops** | Restart the gateway, update IO, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update IO to get the latest bridge version, then re-pair. |
| **Messages not being received** | Verify `WHATSAPP_ALLOWED_USERS` includes the sender's number (with country code, no `+` or spaces). |
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.io/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.io/whatsapp/session` directory contains full session credentials Φ protect it like a password
- Set file permissions: `chmod 700 ~/.io/whatsapp/session`
- Use a **dedicated phone number** for the bot to isolate risk from your personal account
- If you suspect compromise, unlink the device from WhatsApp Φ Settings Φ Linked Devices
