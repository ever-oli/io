...
sidebar_position: 1
title: "Telegram"
description: "Set up IO as a Telegram bot"
---

# Telegram Setup

IO integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "IO") Φ this can be anything
4. Choose a **username** Φ this must be unique and end in `bot` (e.g., `my_io_bot`)
...
2. Send `/mybots`
3. Select your bot
4. Go to **Bot Settings Φ Group Privacy Φ Turn off**
...
IO uses numeric Telegram user IDs to control access. Your user ID is **not** your username Φ it's a number like `123456789`.

**Method 1 (recommended):** Message [@userinfobot](https://t.me/userinfobot) Φ it instantly replies with your user ID.

**Method 2:** Message [@get_id_bot](https://t.me/get_id_bot) Φ another reliable option.
...
You can also set it manually in `~/.io/.env`:
...
Voice messages you send on Telegram are automatically transcribed by IO's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running IO Φ no API key required
- `groq` uses Groq Whisper and requires `GROQ_API_KEY`
- `openai` uses OpenAI Whisper and requires `VOICE_TOOLS_OPENAI_KEY`

### Outgoing Voice (Text-to-Speech)

When the agent generates audio via TTS, it's delivered as native Telegram **voice bubbles** Φ the round, inline-playable kind.

- **OpenAI and ElevenLabs** produce Opus natively Φ no extra setup needed
...
IO works in Telegram group chats with a few considerations:

- **Privacy mode** determines what messages the bot can see (see [Step 3](#step-3-privacy-mode-critical-for-groups))
- When privacy mode is on, **@mention the bot** (e.g., `@my_io_bot what's the weather?`) or **reply to its messages** to interact
- When privacy mode is off (or bot is admin), the bot sees all messages and can participate naturally
- `TELEGRAM_ALLOWED_USERS` still applies Φ only authorized users can trigger the bot, even in groups

## Recent Bot API Features (2024Φ2025)
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `io gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.io/.env`. |
...
> ΦΦ️ This command is potentially dangerous (recursive delete). Reply "yes" to approve.
