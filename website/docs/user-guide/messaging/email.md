...
sidebar_position: 7
title: "Email"
description: "Set up IO as an email assistant via IMAP/SMTP"
---

# Email Setup

IO can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread Φ no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
- **A dedicated email account** for your IO agent (don't use your personal email)
...
4. Copy the 16-character password Φ you'll use this instead of your regular password
...
- **Reply emails** (subject starting with `Re:`) skip the subject prefix Φ the thread context is already established
- **Attachments** are cached locally:
  - Images (JPEG, PNG, GIF, WebP) Φ available to the vision tool
  - Documents (PDF, ZIP, etc.) Φ available for file access
...
Email access follows the same pattern as all other IO platforms:

1. **`EMAIL_ALLOWED_USERS` set** Φ only emails from those addresses are processed
2. **No allowlist set** Φ unknown senders get a pairing code
3. **`EMAIL_ALLOW_ALL_USERS=true`** Φ any sender is accepted (use with caution)
...
| Problem | Solution |
|---------|----------|
| **"IMAP connection failed"** at startup | Verify `EMAIL_IMAP_HOST` and `EMAIL_IMAP_PORT`. Ensure IMAP is enabled on the account. For Gmail, enable it in Settings Φ Forwarding and POP/IMAP. |
...
| **Messages not received** | Check `EMAIL_ALLOWED_USERS` includes the sender's email. Check spam folder Φ some providers flag automated replies. |
| **"Authentication failed"** | For Gmail, you must use an App Password, not your regular password. Ensure 2FA is enabled first. |
| **Duplicate replies** | Ensure only one gateway instance is running. Check `io gateway status`. |
...
:::warning
**Use a dedicated email account.** Don't use your personal email Φ the agent stores the password in `.env` and has full inbox access via IMAP.
:::
...
- The password is stored in `~/.io/.env` Φ protect this file (`chmod 600`)
- IMAP uses SSL (port 993) and SMTP uses STARTTLS (port 587) by default Φ connections are encrypted
...
|----------|----------|---------|-------------|
| `EMAIL_ADDRESS` | Yes | Φ | Agent's email address |
| `EMAIL_PASSWORD` | Yes | Φ | Email password or app password |
| `EMAIL_IMAP_HOST` | Yes | Φ | IMAP server host (e.g., `imap.gmail.com`) |
| `EMAIL_SMTP_HOST` | Yes | Φ | SMTP server host (e.g., `smtp.gmail.com`) |
| `EMAIL_IMAP_PORT` | No | `993` | IMAP server port |
| `EMAIL_SMTP_PORT` | No | `587` | SMTP server port |
| `EMAIL_POLL_INTERVAL` | No | `15` | Seconds between inbox checks |
| `EMAIL_ALLOWED_USERS` | No | Φ | Comma-separated allowed sender addresses |
| `EMAIL_HOME_ADDRESS` | No | Φ | Default delivery target for cron jobs |
