...
IO can generate images from text prompts using FAL.ai's **FLUX 2 Pro** model with automatic 2x upscaling via the **Clarity Upscaler** for enhanced quality.
...
When you ask IO to generate an image:

1. **Generation** Φ Your prompt is sent to the FLUX 2 Pro model (`fal-ai/flux-2-pro`)
2. **Upscaling** Φ The generated image is automatically upscaled 2x using the Clarity Upscaler (`fal-ai/clarity-upscaler`)
3. **Delivery** Φ The upscaled image URL is returned

If upscaling fails for any reason, the original image is returned as a fallback.

## Usage

Simply ask IO to create an image:
...
|-----------|---------|-------|-------------|
| `prompt` | *(required)* | Φ | Text description of the desired image |
| `aspect_ratio` | `"landscape"` | `landscape`, `square`, `portrait` | Image aspect ratio |
| `num_inference_steps` | `50` | 1Φ100 | Number of denoising steps (more = higher quality, slower) |
| `guidance_scale` | `4.5` | 0.1Φ20.0 | How closely to follow the prompt |
| `num_images` | `1` | 1Φ4 | Number of images to generate |
...
- **Requires FAL API key** Φ image generation incurs API costs on your FAL.ai account
- **No image editing** Φ this is text-to-image only, no inpainting or img2img
- **URL-based delivery** Φ images are returned as temporary FAL.ai URLs, not saved locally
- **Upscaling adds latency** Φ the automatic 2x upscale step adds processing time
- **Max 4 images per request** Φ `num_images` is capped at 4
