...
description: "Customize IO's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

IO's personality is fully customizable. `SOUL.md` is the **primary identity** Φ it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` Φ a durable persona file that lives in `IO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets Φ session-level system-prompt overlays

If you want to change who IO is Φ or replace it with an entirely different agent persona Φ edit `SOUL.md`.

## How SOUL.md works now

IO now seeds a default `SOUL.md` automatically in:

```text
~/.io/SOUL.md
```

More precisely, it uses the current instance's `IO_HOME`, so if you run IO with a custom home directory, it will use:
...
- IO creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- IO loads `SOUL.md` only from `IO_HOME`
- IO does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, IO falls back to a built-in default identity
- If `SOUL.md` has content, that content is injected verbatim after security scanning and truncation
- SOUL.md is **not** duplicated in the context files section Φ it appears only once, as the identity
...
If IO loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `IO_HOME`, the personality belongs to the IO instance itself.

That also makes it easier to teach users:
- "Edit `~/.io/SOUL.md` to change IO' default personality."
...
- how IO should handle uncertainty, disagreement, or ambiguity
...
## What IO injects into the prompt

`SOUL.md` content goes directly into slot #1 of the system prompt Φ the agent identity position. No wrapper language is added around it.

The content goes through:
- prompt-injection scanning
- truncation if it is too large

If the file is empty, whitespace-only, or cannot be read, IO falls back to a built-in default identity ("You are IO, an intelligent AI assistant created by Most Wanted Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
IO ships with built-in personalities you can switch to with `/personality`.
...
| **kawaii** | Cute expressions, sparkles, and enthusiasm ΦΦ |
| **catgirl** | Neko-chan with cat-like expressions, nya~ |
| **pirate** | Captain IO, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives IO its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.io/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.io/SOUL.md`
...
1. **SOUL.md** (agent identity Φ or built-in fallback if SOUL.md is unavailable)
...
`SOUL.md` is the foundation Φ everything else builds on top of it.
...
- [SOUL.md Guide](/docs/guides/use-soul-with-io)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how IO speaks
- `display.skin` and `/skin` affect how IO looks in the terminal
