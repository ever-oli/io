...
sidebar_position: 13
title: "RL Training"
description: "Reinforcement learning on agent behaviors with Tinker-Atropos Φ environment discovery, training, and evaluation"
---

# RL Training

IO includes an integrated RL (Reinforcement Learning) training pipeline built on **Tinker-Atropos**. This enables training language models on environment-specific tasks using GRPO (Group Relative Policy Optimization) with LoRA adapters, orchestrated entirely through the agent's tool interface.

## Overview

The RL training system consists of three components:

1. **Atropos** Φ A trajectory API server that coordinates environment interactions, manages rollout groups, and computes advantages
2. **Tinker** Φ A training service that handles model weights, LoRA training, sampling/inference, and optimizer steps
3. **Environments** Φ Python classes that define tasks, scoring, and reward functions (e.g., GSM8K math problems)

The agent can discover environments, configure training parameters, launch training runs, and monitor metrics Φ all through a set of `rl_*` tools.
...
- **TINKER_API_KEY** Φ API key for the Tinker training service
- **WANDB_API_KEY** Φ API key for Weights & Biases metrics tracking
- The `tinker-atropos` submodule (at `tinker-atropos/` relative to the IO root)

```bash
# Set up API keys
io config set TINKER_API_KEY your-tinker-key
io config set WANDB_API_KEY your-wandb-key
```
...
- **Dataset loading** Φ where training data comes from (e.g., HuggingFace datasets)
- **Prompt construction** Φ how to format items for the model
- **Scoring/verification** Φ how to evaluate model outputs and assign rewards
...
- `group_size` Φ Number of completions per item (default: 16)
- `batch_size` Φ Training batch size (default: 128)
- `wandb_name` Φ WandB run name (auto-set to `{env}-{timestamp}`)
- Other environment-specific parameters

**Locked fields** (infrastructure settings, cannot be changed):
- `tokenizer_name` Φ Model tokenizer (e.g., `Qwen/Qwen3-8B`)
- `rollout_server_url` Φ Atropos API URL (`http://localhost:8000`)
- `max_token_length` Φ Maximum token length (8192)
- `max_num_workers` Φ Maximum parallel workers (2048)
- `total_steps` Φ Total training steps (2500)
- `lora_rank` Φ LoRA adapter rank (32)
- `learning_rate` Φ Learning rate (4e-5)
- `max_token_trainer_length` Φ Max tokens for trainer (9000)
...
3. Spawns three processes:
   - **Atropos API server** (`run-api`) Φ trajectory coordination
   - **Tinker trainer** (`launch_training.py`) Φ LoRA training + FastAPI inference server on port 8001
   - **Environment** (`environment.py serve`) Φ the selected environment connecting to Atropos
...
`rl_stop_training()` terminates all three processes in reverse order (environment Φ trainer Φ API). `rl_get_results()` retrieves final WandB metrics and training history.

## Inference Testing

Before committing to a full training run, you can test if an environment works correctly using `rl_test_inference`. This runs a few steps of inference and scoring using OpenRouter Φ no Tinker API needed, just an `OPENROUTER_API_KEY`.
...
- **ServiceClient** Φ Creates training and sampling clients
- **Training client** Φ Handles forward-backward passes with importance sampling loss, optimizer steps (Adam), and weight checkpointing
- **Sampling client** Φ Provides inference using the latest trained weights
...
3. Implement the required methods:
   - `load_dataset()` Φ Load your training data
   - `get_next_item()` Φ Provide the next item to the model
   - `score_answer()` Φ Score model outputs and assign rewards
   - `collect_trajectories()` Φ Collect and return trajectories
4. Optionally define a custom config class inheriting from `BaseEnvConfig`

Study the existing `gsm8k_tinker.py` as a template. The agent can help you create new environments Φ it can read existing environment files, inspect HuggingFace datasets, and write new environment code.
...
```
logs/
ΦΦΦ api_{run_id}.log        # Atropos API server logs
ΦΦΦ trainer_{run_id}.log    # Tinker trainer logs
ΦΦΦ env_{run_id}.log        # Environment process logs
ΦΦΦ inference_tests/        # Inference test results
    ΦΦΦ test_{env}_{model}.jsonl
    ΦΦΦ test_{env}_{model}.log
```
