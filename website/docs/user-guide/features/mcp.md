...
description: "Connect IO to external tool servers via MCP Φ and control exactly which MCP tools IO loads"
---

# MCP (Model Context Protocol)

MCP lets IO connect to external tool servers so the agent can use tools that live outside IO itself Φ GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted IO to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native IO tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want IO to see
...
2. Add an MCP server to `~/.io/config.yaml`:
...
4. Ask IO to use the MCP-backed capability.
...
IO will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints IO connects to directly.
...
- you do not want IO spawning a local subprocess for that integration

## Basic configuration reference

IO reads MCP config from `~/.io/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, IO skips the server entirely |
...
## How IO registers MCP tools

IO prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually Φ IO sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, IO also registers utility tools around MCP resources and prompts:
...
- IO only registers resource utilities if the MCP session actually supports resource operations
- IO only registers prompt utilities if the MCP session actually supports prompt operations
...
You can now control which tools each MCP server contributes to IO.
...
If `enabled: false`, IO skips the server completely and does not even attempt a connection.
...
You can also separately disable IO-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, IO does not create an empty runtime MCP toolset for that server.
...
IO discovers MCP servers at startup and registers their tools into the normal tool registry.
...
For stdio servers, IO does not blindly pass your full shell environment.
...
Then verify your config and restart IO.
...
Because IO now only registers those wrappers when both are true:
