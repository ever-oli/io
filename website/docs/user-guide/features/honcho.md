...
[Honcho](https://honcho.dev) is an AI-native memory system that gives IO persistent, cross-session understanding of users. While IO has built-in memory (`MEMORY.md` and `USER.md`), Honcho adds a deeper layer of **user modeling** Φ learning preferences, goals, communication style, and context across conversations via a dual-peer architecture where both the user and the AI build representations over time.

## Works Alongside Built-in Memory

IO has two memory systems that can work together or be configured separately. In `hybrid` mode (the default), both run side by side Φ Honcho adds cross-session user modeling while local files handle agent-level notes.

| Feature | Built-in Memory | Honcho Memory |
|---------|----------------|---------------|
| Storage | Local files (`~/.io/memories/`) | Cloud-hosted Honcho API |
...
`apiKey` lives at the root because it is a shared credential across all Honcho-enabled tools. All other settings are scoped under `hosts.io`. The `io honcho setup` wizard writes this structure automatically.

Or set the API key as an environment variable:

```bash
io config set HONCHO_API_KEY your-key
```
...
Settings are scoped to `hosts.io` and fall back to root-level globals when the host field is absent. Root-level keys are managed by the user or the honcho CLI -- IO only writes to its own host block (except `apiKey`, which is a shared credential at root).

**Root-level (shared)**

| Field | Default | Description |
|-------|---------|-------------|
| `apiKey` | Φ | Honcho API key (required, shared across all hosts) |
...
| Field | Default | Description |
|-------|---------|-------------|
| `workspace` | `"io"` | Workspace identifier |
| `peerName` | *(derived)* | Your identity name for user modeling |
| `aiPeer` | `"io"` | AI assistant identity name |
...
All host-level fields fall back to the equivalent root-level key if not set under `hosts.io`. Existing configs with settings at root level continue to work.
...
| `honcho` | Honcho only Φ skip local file writes |
...
| `context` | Auto-injected context only Φ Honcho tools hidden |
| `tools` | Honcho tools only Φ no auto-injected context |
...
### IO Config (`~/.io/config.yaml`)

Intentionally minimal Φ most configuration comes from `~/.honcho/config.json`:
...
- **User peer** Φ observed from user messages. Honcho learns preferences, goals, communication style.
- **AI peer** Φ observed from assistant messages (`observe_me=True`). Honcho builds a representation of the agent's knowledge and behavior.
...
Each gateway session (e.g., a Telegram chat, a Discord channel) gets its own Honcho session context. The session key Φ derived from the platform and chat ID Φ is threaded through the entire tool dispatch chain so that Honcho tool calls always execute against the correct session, even when multiple users are messaging concurrently.
...
When Honcho is active, four tools become available. Availability is gated dynamically Φ they are invisible when Honcho is disabled.
...
Semantic search over memory (no LLM). Returns raw excerpts ranked by relevance. Cheaper and faster than `honcho_context` Φ good for factual lookups.

Parameters:
- `query` (string) Φ search query
- `max_tokens` (integer, optional) Φ result token budget
...
Parameters:
- `query` (string) Φ natural language question
- `peer` (string, optional) Φ `"user"` (default) or `"ai"`. Querying `"ai"` asks about the assistant's own history and identity.
...
Parameters:
- `conclusion` (string) Φ the fact to persist

## CLI Commands

```
io honcho setup                        # Interactive setup wizard
io honcho status                       # Show config and connection status
io honcho sessions                     # List directory Φ session name mappings
io honcho map <name>                   # Map current directory to a session name
io honcho peer                         # Show peer names and dialectic settings
io honcho peer --user NAME             # Set user peer name
io honcho peer --ai NAME               # Set AI peer name
io honcho peer --reasoning LEVEL       # Set dialectic reasoning level
io honcho mode                         # Show current memory mode
io honcho mode [hybrid|honcho|local]   # Set memory mode
io honcho tokens                       # Show token budget settings
io honcho tokens --context N           # Set context token cap
io honcho tokens --dialectic N         # Set dialectic char cap
io honcho identity                     # Show AI peer identity
io honcho identity <file>              # Seed AI peer identity from file (SOUL.md, etc.)
io honcho migrate                      # Migration guide: OpenClaw Φ IO + Honcho
```

### Doctor Integration

`io doctor` includes a Honcho section that validates config, API key, and connection status.
...
1. **Conversation history** Φ prior messages are uploaded as an XML transcript file
2. **Memory files** Φ existing `MEMORY.md`, `USER.md`, and `SOUL.md` are uploaded for context
...
- **Personalized responses** Φ Honcho learns how each user prefers to communicate
- **Goal tracking** Φ remembers what users are working toward across sessions
- **Expertise adaptation** Φ adjusts technical depth based on user's background
- **Cross-platform memory** Φ same user understanding across CLI, Telegram, Discord, etc.
- **Multi-user support** Φ each user (via messaging platforms) gets their own user model

:::tip
Honcho is fully opt-in Φ zero behavior change when disabled or unconfigured. All Honcho calls are non-fatal; if the service is unreachable, the agent continues normally.
:::
