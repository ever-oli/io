...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, IO supports **provider routing** Φ fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.io/config.yaml`:
...
- **CLI mode** Φ configured in `~/.io/config.yaml`, loaded at startup
- **Gateway mode** Φ same config file, loaded when the gateway starts

The routing config is read from `config.yaml` and passed as parameters when creating the `AIAgent`:

```
providers_allowed  Φ from provider_routing.only
providers_ignored  Φ from provider_routing.ignore
providers_order    Φ from provider_routing.order
provider_sort      Φ from provider_routing.sort
provider_require_parameters Φ from provider_routing.require_parameters
provider_data_collection    Φ from provider_routing.data_collection
```
