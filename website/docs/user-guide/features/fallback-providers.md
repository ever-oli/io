...
IO has two separate fallback systems that keep your sessions running when providers hit issues:

1. **Primary model fallback** Φ automatically switches to a backup provider:model when your main model fails
2. **Auxiliary task fallback** Φ independent provider resolution for side tasks like vision, compression, and web extraction

Both are optional and work independently.

## Primary Model Fallback

When your main LLM provider encounters errors Φ rate limits, server overload, auth failures, connection drops Φ IO can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.io/config.yaml`:
...
| Nous Portal | `nous` | `io login` (OAuth) |
| OpenAI Codex | `openai-codex` | `io model` (ChatGPT OAuth) |
...
- **Rate limits** (HTTP 429) Φ after exhausting retry attempts
- **Server errors** (HTTP 500, 502, 503) Φ after exhausting retry attempts
- **Auth failures** (HTTP 401, 403) Φ immediately (no point retrying)
- **Not found** (HTTP 404) Φ immediately
- **Invalid responses** Φ when the API returns malformed or empty responses repeatedly
...
The switch is seamless Φ your conversation history, tool calls, and context are preserved. The agent continues from exactly where it left off, just using a different model.
...
|---------|-------------------|
| CLI sessions | Φ |
| Messaging gateway (Telegram, Discord, etc.) | Φ |
| Subagent delegation | ΦΦ (subagents do not inherit fallback config) |
| Cron jobs | ΦΦ (run with a fixed provider) |
| Auxiliary tasks (vision, compression) | ΦΦ (use their own provider chain Φ see below) |

:::tip
There are no environment variables for `fallback_model` Φ it is configured exclusively through `config.yaml`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
IO uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), IO tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter Φ Nous Portal Φ Custom endpoint Φ Codex OAuth Φ
API-key providers (z.ai, Kimi, MiniMax, Anthropic) Φ give up
```

**For vision tasks:**

```text
Main provider (if vision-capable) Φ OpenRouter Φ Nous Portal Φ
Codex OAuth Φ Anthropic Φ Custom endpoint Φ give up
```

If the resolved provider fails at call time, IO also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
All three Φ auxiliary, compression, fallback Φ work the same way: set `provider` to pick who handles the request, `model` to pick which model, and `base_url` to point at a custom endpoint (overrides provider).
...
| `"codex"` | Force Codex OAuth | `io model` Φ Codex |
...
`base_url` takes precedence over `provider`. IO uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, IO drops middle conversation turns without generating a summary rather than failing the session.
...
|---------|-------------------|----------------|
| Main agent model | `fallback_model` in config.yaml Φ one-shot failover on errors | `fallback_model:` (top-level) |
