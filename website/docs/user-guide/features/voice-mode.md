...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with IO Φ CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

IO supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with IO](/docs/guides/use-voice-mode-with-io).

## Prerequisites

Before using voice features, make sure you have:

1. **IO installed** Φ `pip install io` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** Φ run `io model` or set your preferred provider credentials in `~/.io/.env`
3. **A working base setup** Φ run `io` to verify the agent responds to text before enabling voice

:::tip
The `~/.io/` directory and default `config.yaml` are created automatically the first time you run `io`. You only need to create `~/.io/.env` manually for API keys.
:::
...
pip install io[messaging]

# Premium TTS (ElevenLabs)
pip install io[tts-premium]
...
| **ffmpeg** | Audio format conversion (MP3 Φ Opus, PCM Φ WAV) | All platforms |
...
```bash
# Speech-to-Text Φ local provider needs NO key at all
# pip install faster-whisper          # Free, runs locally, recommended
GROQ_API_KEY=your-key                 # Groq Whisper Φ fast, free tier (cloud)
VOICE_TOOLS_OPENAI_KEY=your-key       # OpenAI Whisper Φ paid (cloud)

# Text-to-Speech (optional Φ Edge TTS and NeuTTS work without any key)
ELEVENLABS_API_KEY=***           # ElevenLabs Φ premium quality
...
1. Start the CLI with `io` and enable voice mode with `/voice on`
2. **Press Ctrl+B** Φ a beep plays (880Hz), recording starts
3. **Speak** Φ a live audio level bar shows your input: `Φ [ΦΦΦΦΦΦΦΦ] Φ`
4. **Stop speaking** Φ after 3 seconds of silence, recording auto-stops
...
8. Recording **automatically restarts** Φ speak again without pressing any key
...
:::tip
The record key is configurable via `voice.record_key` in `~/.io/config.yaml` (default: `ctrl+b`).
:::
...
1. **Speech confirmation** Φ waits for audio above the RMS threshold (200) for at least 0.3s, tolerating brief dips between syllables
2. **End detection** Φ once speech is confirmed, triggers after 3.0 seconds of continuous silence
...
When TTS is enabled, the agent speaks its reply **sentence-by-sentence** as it generates text Φ you don't wait for the full response:
...
```bash
io gateway        # Start the gateway (connects to configured platforms)
io gateway setup  # Interactive setup wizard for first-time configuration
```
...
|------|------------|-----------------|-------|
| **Direct Message (DM)** | Open the bot's profile Φ "Message" | No | Works immediately |
| **Server Channel** | Type in a text channel where the bot is present | Yes (`@botname`) | Bot must be invited to the server |

**DM (recommended for personal use):** Just open a DM with the bot and type Φ no @mention needed. Voice replies and all commands work the same as in channels.

**Server channels:** The bot only responds when you @mention it (e.g. `@iobyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.io/.env`:
```bash
...
|----------|--------|-------|
| **Telegram** | Voice bubble (Opus/OGG) | Plays inline in chat. ffmpeg converts MP3 Φ Opus if needed |
...
Go to the [Discord Developer Portal](https://discord.com/developers/applications) Φ your application Φ **Installation** Φ **Default Install Settings** Φ **Guild Install**:
...
In the [Developer Portal](https://discord.com/developers/applications) Φ your application Φ **Bot** Φ **Privileged Gateway Intents**, enable all three:
...
All three are required for full voice channel functionality. **Server Members Intent** is especially critical Φ without it, the bot cannot identify who is speaking in the voice channel.
...
# STT Φ local provider needs no key (pip install faster-whisper)
# GROQ_API_KEY=your-key            # Alternative: cloud-based, fast, free tier

# TTS Φ optional. Edge TTS and NeuTTS need no key.
...
```bash
io gateway        # Start with existing configuration
```
...
2. **Detects silence** Φ 1.5s of silence after at least 0.5s of speech triggers processing
...
# pip install faster-whisper        # Free local STT Φ no API key needed
...
2. Or use DMs instead Φ no mention needed
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.io/.env`
...
- The bot needs a SPEAKING event from Discord before it can map your audio Φ start speaking within a few seconds of joining
...
- Review gateway logs: `tail -f ~/.io/logs/gateway.log`

### Bot responds in text but not in voice channel

- TTS provider may be failing Φ check API key and quota
