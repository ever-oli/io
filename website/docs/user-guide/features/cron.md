...
Schedule tasks to run automatically with natural language or cron expressions. IO exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. IO disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
io cron create "every 2h" "Check server status"
io cron create "every 1h" "Summarize new feed items" --skill blogwatcher
io cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
IO will use the unified `cronjob` tool internally.
...
```bash
io cron edit <job_id> --schedule "every 4h"
io cron edit <job_id> --prompt "Use the revised task"
io cron edit <job_id> --skill blogwatcher --skill find-nearby
io cron edit <job_id> --add-skill find-nearby
io cron edit <job_id> --remove-skill blogwatcher
io cron edit <job_id> --clear-skills
```
...
```bash
io cron list
io cron pause <job_id>
io cron resume <job_id>
io cron run <job_id>
io cron remove <job_id>
io cron status
io cron tick
```
...
- `pause` Φ keep the job but stop scheduling it
- `resume` Φ re-enable the job and compute the next future run
- `run` Φ trigger the job on the next scheduler tick
- `remove` Φ delete it entirely
...
sudo io gateway install --system   # Linux: boot-time system service for servers
io gateway             # Or run in foreground
...
1. loads jobs from `~/.io/cron/jobs.json`
...
A file lock at `~/.io/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.io/cron/output/`) | Default on CLI |
...
The agent's final response is automatically delivered Φ you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, IO skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.

### Relative delays (one-shot)

```text
30m     Φ Run once in 30 minutes
2h      Φ Run once in 2 hours
1d      Φ Run once in 1 day
```

### Intervals (recurring)

```text
every 30m    Φ Every 30 minutes
every 2h     Φ Every 2 hours
every 1d     Φ Every day
```

### Cron expressions

```text
0 9 * * *       Φ Daily at 9:00 AM
0 9 * * 1-5     Φ Weekdays at 9:00 AM
0 */6 * * *     Φ Every 6 hours
30 8 1 * *      Φ First of every month at 8:30 AM
0 0 * * 0       Φ Every Sunday at midnight
```

### ISO timestamps

```text
2026-03-15T09:00:00    Φ One-time at March 15, 2026 9:00 AM
```
...
Jobs are stored in `~/.io/cron/jobs.json`. Output from job runs is saved to `~/.io/cron/output/{job_id}/{timestamp}.md`.
