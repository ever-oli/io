...
description: "Generate agent trajectories at scale Φ parallel processing, checkpointing, and toolset distributions"
---

# Batch Processing

Batch processing lets you run the IO agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** Φ producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
```text
data/my_run/
ΦΦΦ trajectories.jsonl    # Combined final output (all batches merged)
ΦΦΦ batch_0.jsonl         # Individual batch results
ΦΦΦ batch_1.jsonl
ΦΦΦ ...
ΦΦΦ checkpoint.json       # Resume checkpoint
ΦΦΦ statistics.json       # Aggregate tool usage stats
```
...
- **Failed prompts:** Only successfully completed prompts are marked as done Φ failed prompts will be retried on resume
