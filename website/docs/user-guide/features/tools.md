...
description: "Overview of IO's tools Φ what's available, how toolsets work, and terminal backends"
---
...
IO ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, Honcho memory, and more.
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `io-cli`, `io-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
Recommended for security Φ agent can't modify its own code:
...
# Configure
io config set terminal.backend singularity
io config set terminal.singularity_image ~/python.sif
```
...
modal setup
io config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.io/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.io/.env`.
:::
