...
sidebar_position: 2
title: "Configuration"
description: "Configure IO Φ config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.io/` directory for easy access.

## Directory Structure

```text
~/.io/
ΦΦΦ config.yaml     # Settings (model, terminal, TTS, compression, etc.)
ΦΦΦ .env            # API keys and secrets
ΦΦΦ auth.json       # OAuth provider credentials (Nous Portal, etc.)
ΦΦΦ SOUL.md         # Primary agent identity (slot #1 in system prompt)
ΦΦΦ memories/       # Persistent memory (MEMORY.md, USER.md)
ΦΦΦ skills/         # Agent-created skills (managed via skill_manage tool)
ΦΦΦ cron/           # Scheduled jobs
ΦΦΦ sessions/       # Gateway sessions
ΦΦΦ logs/           # Logs (errors.log, gateway.log Φ secrets auto-redacted)
```
...
io config edit         # Open config.yaml in your editor
io config set KEY VAL  # Set a specific value
io config check        # Check for missing options (after updates)
io config migrate      # Interactively add missing options

# Examples:
io config set model anthropic/claude-opus-4
io config set terminal.backend docker
io config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `io config set` command automatically routes values to the right file Φ API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** Φ e.g., `io chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.io/config.yaml`** Φ the primary config file for all non-secret settings
3. **`~/.io/.env`** Φ fallback for env vars; **required** for secrets (API keys, tokens, passwords)
4. **Built-in defaults** Φ hardcoded safe defaults when nothing else is set
...
You need at least one way to connect to an LLM. Use `io model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `io model` (OAuth, subscription-based) |
| **OpenAI Codex** | `io model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `io model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `io model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `io model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.io/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.io/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.io/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.io/.env` (provider: `kimi-coding`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.io/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.io/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.io/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.io/.env` (provider: `kilocode`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.io/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.io/.env` (provider: `opencode-go`) |
| **Custom Endpoint** | `io model` (saved in `config.yaml`) or `OPENAI_BASE_URL` + `OPENAI_API_KEY` in `~/.io/.env` |

:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). IO stores the resulting credentials in its own auth store under `~/.io/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model Φ by default Gemini Flash via OpenRouter. An `OPENROUTER_API_KEY` enables these tools automatically. You can also configure which model and provider these tools use Φ see [Auxiliary Models](#auxiliary-models) below.
:::

### Anthropic (Native)

Use Claude models directly through the Anthropic API Φ no OpenRouter proxy needed. Supports three auth methods:

```bash
# With an API key (pay-per-token)
export ANTHROPIC_API_KEY=***
io chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `io model`
# IO will use Claude Code's credential store directly when available
io model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
io chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
io chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `io model`, IO prefers Claude Code's own credential store over copying the token into `~/.io/.env`. That keeps refreshable Claude credentials refreshable.
...
IO supports GitHub Copilot as a first-class provider with two modes:

**`copilot` Φ Direct Copilot API** (recommended). Uses your GitHub Copilot subscription to access GPT-5.x, Claude, Gemini, and other models through the Copilot API.

```bash
io chat --provider copilot --model gpt-5.4
```
...
If no token is found, `io model` offers an **OAuth device code login** Φ the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `io model` Φ GitHub Copilot Φ Login with GitHub |
| Fine-grained PAT | `github_pat_` | GitHub Settings Φ Developer settings Φ Fine-grained tokens (needs **Copilot Requests** permission) |
| GitHub App token | `ghu_` | Via GitHub App installation |

If your `gh auth token` returns a `ghp_*` token, use `io model` to authenticate via OAuth instead.
:::
...
**`copilot-acp` Φ Copilot ACP agent backend**. Spawns the local Copilot CLI as a subprocess:

```bash
io chat --provider copilot-acp --model copilot-acp
...
| `IO_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `IO_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
io chat --provider zai --model glm-4-plus
# Requires: GLM_API_KEY in ~/.io/.env

# Kimi / Moonshot AI
io chat --provider kimi-coding --model moonshot-v1-auto
# Requires: KIMI_API_KEY in ~/.io/.env

# MiniMax (global endpoint)
io chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.io/.env

# MiniMax (China endpoint)
io chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.io/.env

# Alibaba Cloud / DashScope (Qwen models)
io chat --provider alibaba --model qwen-plus
# Requires: DASHSCOPE_API_KEY in ~/.io/.env
```
...
IO works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point IO at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`io model` and the manual `.env` approach end up in the same runtime path. If you save a custom endpoint through `io model`, IO persists the provider + base URL in `config.yaml` so later sessions keep using that endpoint even if `OPENAI_BASE_URL` is not exported in your current shell.

Everything below follows this same pattern Φ just change the URL, key, and model name.

---

### Ollama Φ Local Models, Zero Config
...
Ollama's OpenAI-compatible endpoint supports chat completions, streaming, and tool calling (for supported models). No GPU required for smaller models Φ Ollama handles CPU inference automatically.
...
### vLLM Φ High-Performance GPU Inference
...
vLLM supports tool calling, structured output, and multi-modal models. Use `--enable-auto-tool-choice` and `--tool-call-parser io` for IO-format tool calling with NousResearch models.

---

### SGLang Φ Fast Serving with RadixAttention
...
### llama.cpp / llama-server Φ CPU & Metal Inference
...
### LiteLLM Proxy Φ Multi-Provider Gateway
...
### ClawRouter Φ Cost-Optimized Routing
...
IO automatically detects your model's context length by querying the endpoint's `/v1/models` response. For most setups this works out of the box. If detection fails (the model name doesn't match, the endpoint doesn't expose `/v1/models`, etc.), IO falls back to a high default and probes downward on context-length errors.
...
This takes highest priority Φ it overrides auto-detection, cached values, and hardcoded defaults.
...
:::tip
You can switch between providers at any time with `io model` Φ no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
1. Clone and start the Firecrawl Docker stack (5 containers: API, Playwright, Redis, RabbitMQ, PostgreSQL Φ requires ~4-8 GB RAM):
   ```bash
...
2. Point IO at your instance (no API key needed):
   ```bash
   io config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.io/config.yaml`:
...
Configure a backup provider:model that IO switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
:::tip
Fallback is configured exclusively through `config.yaml` Φ there are no environment variables for it. For full details on when it triggers, supported providers, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/user-guide/features/fallback-providers).
:::

## Smart Model Routing

Optional cheap-vs-strong routing lets IO keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, IO may route it to `cheap_model`
- If the turn looks complex, IO stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, IO falls back to the primary model automatically
...
terminal:
  ...
  # Persistent shell Φ keep a long-lived bash process across commands
...
- **Docker backend**
  ...
  - IO needs to be able to find the `docker` CLI. It checks your `$PATH` first and also probes common Docker Desktop install locations on macOS. Run:
    ```bash
    docker version
    ```
    If this fails, fix your Docker installation or switch back to the local backend:
    ```bash
    io config set terminal.backend local
    ```
...
- **SSH backend**
  - Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set, for example:
    ...
    ```
  - If either value is missing, IO will log a clear error and refuse to use the SSH backend.

- **Modal backend**
  - You need either a `MODAL_TOKEN_ID` environment variable or a `~/.modal.toml` config file.
  - If neither is present, the backend check fails and IO will report that the Modal backend is not available.
...
IO resolves each listed variable from your current shell first, then falls back to `~/.io/.env` if it was saved with `io config set`.
...
Docker sandboxes stay isolated by default. IO does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch IO from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched IO from

Use the opt-in only when you intentionally want the container to work on live host files.

### Persistent Shell

By default, each terminal command runs in its own subprocess Φ working directory, environment variables, and shell variables reset between commands. When **persistent shell** is enabled, a single long-lived bash process is kept alive across `execute()` calls so that state survives between commands.
...
```yaml
terminal:
  persistent_shell: true   # default Φ enables persistent shell for SSH
```

To disable:

```bash
io config set terminal.persistent_shell false
```
...
```yaml
worktree: true    # Always create a worktree (same as io -w)
# worktree: false # Default Φ only when -w flag is passed
```
...
IO automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call Φ you can point it at any provider or endpoint.
...
**Default (auto-detect) Φ no configuration needed:**
```yaml
compression:
  enabled: true
  threshold: 0.50
```
Uses the first available provider (OpenRouter Φ Nous Φ Codex) with Gemini Flash.
...
Warnings are injected into the last tool result's JSON (as a `_budget_warning` field) rather than as separate messages Φ this preserves prompt caching and doesn't disrupt the conversation structure.
...
IO uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection Φ you don't need to configure anything.

### The universal config pattern

Every model slot in IO Φ auxiliary tasks, compression, fallback Φ uses the same three knobs:
...
When `base_url` is set, IO ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, IO uses that provider's built-in auth and base URL.
...
:::info
Context compression has its own top-level `compression:` block with `summary_provider`, `summary_model`, and `summary_base_url` Φ see [Context Compression](#context-compression) above. The fallback model uses a `fallback_model:` block Φ see [Fallback Model](#fallback-model) above. All three follow the same provider/model/base_url pattern.
:::
...
Or via environment variable (in `~/.io/.env`):
...
|----------|-------------|-------------|
| `"auto"` | Best available (default). Vision tries OpenRouter Φ Nous Φ Codex. | Φ |
| `"openrouter"` | Force OpenRouter Φ routes to any model (Gemini, GPT-4o, Claude, etc.) | `OPENROUTER_API_KEY` |
| `"nous"` | Force Nous Portal | `io login` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `io model` Φ Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `io model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, IO uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
**Using Codex OAuth** (ChatGPT Pro/Plus account Φ no API key needed):
```yaml
auxiliary:
  vision:
...
`provider: "main"` follows the same custom endpoint IO uses for normal chat. That endpoint can be set directly with `OPENAI_BASE_URL`, or saved once through `io model` and persisted in `config.yaml`.

:::tip
If you use Codex OAuth as your main model provider, vision works automatically Φ no extra configuration needed. Codex is included in the auto-detection chain for vision.
:::

:::warning
**Vision requires a multimodal model.** If you set `provider: "main"`, make sure your endpoint supports multimodal/vision Φ otherwise image analysis will fail.
:::

### Environment Variables (legacy)

Auxiliary models can also be configured via environment variables. However, `config.yaml` is the preferred method Φ it's easier to manage and supports all options including `base_url` and `api_key`.
...
:::tip
Run `io config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
When unset (default), reasoning effort defaults to "medium" Φ a balanced level that works well for most tasks. Setting a value overrides it Φ higher reasoning effort gives better results on complex tasks at the cost of more tokens and latency.
...
display:
  ...
  theme_mode: auto        # auto | light | dark Φ color scheme for skin-aware rendering
...
This works with any skin Φ built-in or custom. Skin authors can provide `colors_light` in their skin definition for optimal light-terminal appearance.

| Mode | What you see |
|------|-------------|
| `off` | Silent Φ just the final response |
...
Hashes are deterministic Φ the same user always maps to the same hash, so the model can still distinguish between users in group chats. Routing and delivery use the original values internally.
...
If the requested provider is unavailable, IO falls back automatically in this order: `local` Φ `groq` Φ `openai`.
...
streaming:
  ...
  cursor: " Φ"            # Cursor shown during streaming
```
...
:::note
Streaming is disabled by default. Enable it in `~/.io/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want IO to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. IO still keys DMs by chat/DM ID as usual.
...
Control what IO does when an unknown user sends a direct message:
...
- `pair` is the default. IO denies access, but replies with a one-time pairing code in DMs.
...
Define custom commands that run shell commands without invoking the LLM Φ zero token usage, instant execution. Especially useful from messaging platforms (Telegram, Discord, etc.) for quick server checks or utility scripts.

```yaml
quick_commands:
  status:
    type: exec
    command: systemctl status io
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.io/io && git pull && pip install -e .
  gpu:
    type: exec
    command: nvidia-smi --query-gpu=name,utilization.gpu,memory.used,memory.total --format=csv,noheader
```

Usage: type `/status`, `/disk`, `/update`, or `/gpu` in the CLI or any messaging platform. The command runs locally on the host and returns the output directly Φ no LLM call, no tokens consumed.

- **30-second timeout** Φ long-running commands are killed with an error message
- **Priority** Φ quick commands are checked before skill commands, so you can override skill names
- **Autocomplete** Φ quick commands are resolved at dispatch time and are not shown in the built-in slash-command autocomplete tables
- **Type** Φ only `exec` is supported (runs a shell command); other types show an error
- **Works everywhere** Φ CLI, Telegram, Discord, Slack, WhatsApp, Signal, Email, Home Assistant
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.io/browser_recordings/
```
...
Control how IO handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `IO_YOLO_MODE=true`. **Use with caution.** |

Smart mode is particularly useful for reducing approval fatigue Φ it lets the agent work more autonomously on safe operations while still catching genuinely destructive commands.
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: io --checkpoints)
...
**Subagent provider:model override:** By default, subagents inherit the parent agent's provider and model. Set `delegation.provider` and `delegation.model` to route subagents to a different provider:model pair Φ e.g., use a cheap/fast model for narrowly-scoped subtasks while your primary agent runs an expensive reasoning model.

**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, IO falls back to `OPENAI_API_KEY` only.

The delegation provider uses the same credential resolution as CLI/gateway startup. All configured providers are supported: `openrouter`, `nous`, `copilot`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`. When a provider is set, the system automatically resolves the correct base URL, API key, and API mode Φ no manual credential wiring needed.

**Precedence:** `delegation.base_url` in config Φ `delegation.provider` in config Φ parent provider (inherited). `delegation.model` in config Φ parent model (inherited). Setting just `model` without `provider` changes only the model name while keeping the parent's credentials (useful for switching models within the same provider like OpenRouter).
...
IO uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** Φ defines who the agent is (slot #1 in the system prompt) | `~/.io/SOUL.md` or `$IO_HOME/SOUL.md` |
...
- If SOUL.md is missing, empty, or cannot be loaded, IO falls back to a built-in default identity.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- IO automatically seeds a default `SOUL.md` if one does not already exist.
