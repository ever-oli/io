...
IO automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.

## How Sessions Work

Every conversation Φ whether from the CLI, Telegram, Discord, WhatsApp, or Slack Φ is stored as a session with full message history. Sessions are tracked in two complementary systems:

1. **SQLite database** (`~/.io/state.db`) Φ structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.io/sessions/`) Φ raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`io` or `io chat`) |
...
io -c "my project"   # Φ resumes "my project #3"
```
...
io --resume 20250305_091523_a1b2c3d4
io -r 20250305_091523_a1b2c3d4

# Resume by title
io --resume "refactoring auth"

# Or with the chat subcommand
io chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `io sessions list`.

### Conversation Recap on Resume

When you resume a session, IO displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a IO session." />
...
The recap:
- Shows **user messages** (gold `Φ`) and **assistant responses** (green `Φ`)
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.io/config.yaml`:
...
:::tip
Session IDs follow the format `YYYYMMDD_HHMMSS_<8-char-hex>`, e.g. `20250305_091523_a1b2c3d4`. You can resume by ID or by title Φ both work with `-c` and `-r`.
:::
...
```bash
io sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```

### Title Rules

- **Unique** Φ no two sessions can share the same title
- **Max 100 characters** Φ keeps listing output clean
- **Sanitized** Φ control characters, zero-width chars, and RTL overrides are stripped automatically
- **Normal Unicode is fine** Φ emoji, CJK, accented characters all work

### Auto-Lineage on Compression

When a session's context is compressed (manually via `/compress` or automatically), IO creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" Φ "my project #2" Φ "my project #3"
```

When you resume by name (`io -c "my project"`), it automatically picks the most recent session in the lineage.

### /title in Messaging Platforms

The `/title` command works in all gateway platforms (Telegram, Discord, Slack, WhatsApp):

- `/title My Research` Φ set the session title
- `/title` Φ show the current title

## Session Management Commands

IO provides a full set of session management commands via `io sessions`:
...
# Filter by platform
io sessions list --source telegram

# Show more sessions
io sessions list --limit 50
```
...
ΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦ
refactoring auth       Help me refactor the auth module please   2h ago        20250305_091523_a
my project #3          Can you check the test failures?          yesterday     20250304_143022_e
Φ                      What's the weather in Las Vegas?          3d ago        20250303_101500_f
```
...
ΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦΦ
...
io sessions export backup.jsonl

# Export sessions from a specific platform
io sessions export telegram-history.jsonl --source telegram

# Export a single session
io sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
io sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
io sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
io sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
io sessions prune --older-than 30

# Only prune sessions from a specific platform
io sessions prune --source telegram --older-than 60

# Skip confirmation
io sessions prune --older-than 30 --yes
```
...
For deeper analytics Φ token usage, cost estimates, tool breakdown, and activity patterns Φ use [`io insights`](/docs/reference/cli-commands#io-insights).
...
When IO cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, IO uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to IO in the same Discord channel without sharing transcript history
...
- **idle** Φ reset after N minutes of inactivity
- **daily** Φ reset at a specific hour each day
- **both** Φ reset on whichever comes first (idle or daily)
- **none** Φ never auto-reset
...
|------|------|-------------|
| SQLite database | `~/.io/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.io/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.io/sessions/sessions.json` | Maps session keys to active session IDs |
...
- **sessions** Φ session metadata (id, source, user_id, model, title, timestamps, token counts). Titles have a unique index (NULL titles allowed, only non-NULL must be unique).
- **messages** Φ full message history (role, content, tool_calls, tool_name, token_count)
- **messages_fts** Φ FTS5 virtual table for full-text search across message content
