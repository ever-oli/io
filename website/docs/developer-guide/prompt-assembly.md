...
description: "How IO builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

IO deliberately separates:
...
1. agent identity Φ `SOUL.md` from `IO_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
7. skills index
8. context files (`AGENTS.md`, `.cursorrules`, `.cursor/rules/*.mdc`) Φ SOUL.md is **not** included here when it was already loaded as the identity in step 1
