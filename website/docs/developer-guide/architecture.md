...
sidebar_position: 1
title: "Architecture"
description: "IO internals Φ major subsystems, execution paths, and where to read next"
---

# Architecture

This page is the top-level map of IO internals. The project has grown beyond a single monolithic loop, so the best way to understand it is by subsystem.

## High-level structure

```text
io/
ΦΦΦ run_agent.py              # AIAgent core loop
ΦΦΦ cli.py                    # interactive terminal UI
ΦΦΦ model_tools.py            # tool discovery/orchestration
ΦΦΦ toolsets.py               # tool groupings and presets
ΦΦΦ io_agent/session.py           # SQLite session/state database
ΦΦΦ batch_runner.py           # batch trajectory generation
Φ
ΦΦΦ agent/                    # prompt building, compression, caching, metadata, trajectories
ΦΦΦ io_cli/               # command entrypoints, auth, setup, models, config, doctor
ΦΦΦ tools/                    # tool implementations and terminal environments
ΦΦΦ gateway/                  # messaging gateway, session routing, delivery, pairing, hooks
ΦΦΦ cron/                     # scheduled job storage and scheduler
ΦΦΦ honcho_integration/       # Honcho memory integration
ΦΦΦ acp_adapter/              # ACP editor integration server
ΦΦΦ acp_registry/             # ACP registry manifest + icon
ΦΦΦ environments/             # IO RL / benchmark environment framework
ΦΦΦ skills/                   # bundled skills
ΦΦΦ optional-skills/          # official optional skills
ΦΦΦ tests/                    # test suite
```
...
IO has a shared runtime provider resolver used by CLI, gateway, cron, ACP, and auxiliary calls.
...
ACP exposes IO as an editor-native agent over stdio/JSON-RPC.
...
IO ships a full environment framework for evaluation, RL integration, and SFT data generation.
...
The older mental model of IO as Φone OpenAI-compatible chat loop plus some toolsΦ is no longer sufficient. Current IO includes:
