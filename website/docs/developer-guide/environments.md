...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the IO Atropos integration"
---

# Environments, Benchmarks & Data Generation

IO includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:

1. **RL Training** Φ Train language models on multi-turn agentic tasks with GRPO
2. **Benchmarks** Φ Evaluate models on standardised agentic benchmarks
3. **Data Generation** Φ Generate SFT training data from agent rollouts
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for IO/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- IOAgentBaseEnv
    IOAgentBaseEnv <|-- TerminalTestEnv
    IOAgentBaseEnv <|-- IOSweEnv
    IOAgentBaseEnv <|-- TerminalBench2EvalEnv
...
- **Server management** Φ connects to OpenAI-compatible APIs (VLLM, SGLang, OpenRouter)
- **Worker scheduling** Φ parallel rollout coordination
- **Wandb integration** Φ metrics logging and rollout visualisation
- **CLI interface** Φ three subcommands: `serve`, `process`, `evaluate`
- **Eval logging** Φ `evaluate_log()` saves results to JSON + JSONL

### IOAgentBaseEnv

The io layer (`environments/io_base_env.py`). Adds:
- **Terminal backend configuration** Φ sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** Φ `_resolve_tools_for_group()` calls io's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** Φ `collect_trajectory()` runs `IOAgentLoop` and scores the result
- **Two-phase operation** Φ Phase 1 (OpenAI server) for eval/SFT, Phase 2 (VLLM ManagedServer) for full RL with logprobs
- **Async safety patches** Φ monkey-patches Modal backend to work inside Atropos's event loop

### Concrete Environments

Your environment inherits from `IOAgentBaseEnv` and implements five methods:
...
| `compute_reward(item, result, ctx)` | Score the rollout (0.0Φ1.0) |
...
`IOAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as io's main loop:
...
| **Generic** | `call_tool(name, args)` Φ escape hatch for any io tool |
| **Cleanup** | `cleanup()` Φ release all resources |
...
parser = get_parser("io")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `io`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.

In Phase 1 (OpenAI server type), parsers are not needed Φ the server handles tool call parsing natively.
...
| **Cost** | ~$50Φ200 for full eval (parallel execution) |
| **Time** | ~2Φ4 hours |
...
**100 difficulty-calibrated tasks** Φ a faster proxy for TerminalBench2.
...
TBLite is a thin subclass of TerminalBench2 Φ only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [NousResearch/openthoughts-tblite](https://huggingface.co/datasets/NousResearch/openthoughts-tblite).

### YC-Bench

**Long-horizon strategic benchmark** Φ the agent plays CEO of an AI startup.
...
| **Cost** | ~$50Φ200 for full eval |
| **Time** | ~3Φ6 hours |

```bash
# Install yc-bench (optional dependency)
pip install "io[yc-bench]"
...
```bash
python environments/io_swe_env/io_swe_env.py serve \
...
### `serve` Φ Connect to Atropos for RL training
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"io"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(IOAgentEnvConfig):
...
4. **Implement** `rollout_and_score_eval(eval_item)` Φ the per-item agent loop + scoring
5. **Implement** `evaluate()` Φ orchestrates all runs, computes aggregate metrics
...
### IOAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which io toolsets to enable |
...
| `tool_call_parser` | `str` | `"io"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "io[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "io[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
- No Modal needed Φ runs with local terminal backend

### For RL training

- `TINKER_API_KEY` Φ API key for the [Tinker](https://tinker.computer) training service
- `WANDB_API_KEY` Φ for Weights & Biases metrics tracking
...
```
environments/
ΦΦΦ io_base_env.py          # Abstract base class (IOAgentBaseEnv)
ΦΦΦ agent_loop.py               # Multi-turn agent engine (IOAgentLoop)
ΦΦΦ tool_context.py             # Per-rollout tool access for reward functions
ΦΦΦ patches.py                  # Async-safety patches for Modal backend
Φ
ΦΦΦ tool_call_parsers/          # Phase 2 client-side parsers
Φ   ΦΦΦ io_parser.py        # IO/ChatML <tool_call> format
Φ   ΦΦΦ mistral_parser.py       # Mistral [TOOL_CALLS] format
Φ   ΦΦΦ llama_parser.py         # Llama 3 JSON tool calling
Φ   ΦΦΦ qwen_parser.py          # Qwen format
Φ   ΦΦΦ deepseek_v3_parser.py   # DeepSeek V3 format
Φ   ΦΦΦ ...                     # + kimi_k2, longcat, glm45/47, etc.
Φ
ΦΦΦ terminal_test_env/          # Stack validation (inline tasks)
ΦΦΦ io_swe_env/             # SWE-bench training environment
Φ
ΦΦΦ benchmarks/                 # Evaluation benchmarks
    ΦΦΦ terminalbench_2/        # 89 terminal tasks, Modal sandboxes
    ΦΦΦ tblite/                 # 100 calibrated tasks (fast TB2 proxy)
    ΦΦΦ yc_bench/               # Long-horizon strategic benchmark
```
