...
The ACP adapter wraps IO' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
io acp / io-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.io/.env
  -> configure stderr logging
  -> construct IOACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> IO `once`
- `allow_always` -> IO `always`
- reject options -> IO `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps IO tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["io-acp"])
...
Instead it reuses IO' runtime resolver:

- `acp_adapter/auth.py`
- `io_cli/runtime_provider.py`

So ACP advertises and uses the currently configured IO provider/credentials.
...
- `tests/acp/` Φ ACP test suite
- `toolsets.py` Φ `io-acp` toolset definition
- `io_cli/main.py` Φ `io acp` CLI subcommand
- `pyproject.toml` Φ `[acp]` optional dependency + `io-acp` script
