...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to IO Φ schemas, handlers, registration, and toolsets"
---
...
1. **`tools/your_tool.py`** Φ handler, schema, check function, `registry.register()` call
2. **`toolsets.py`** Φ add tool name to `_IO_CORE_TOOLS` (or a specific toolset)
3. **`model_tools.py`** Φ add `"tools.your_tool"` to the `_discover_tools()` list
...
- The `check_fn` is called when building tool definitions Φ if it returns `False`, the tool is silently excluded
...
The registry handles async bridging transparently Φ you never call `asyncio.run()` yourself.
...
If your tool requires an API key, add it to `io_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `io_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `io chat -q "Use the weather tool for London"`
