...
description: "How IO stores sessions in SQLite, maintains lineage, and exposes recall/search"
---

# Session Storage

IO uses a SQLite-backed session store as the main source of truth for historical conversation state.
...
When IO compresses a conversation, it can continue in a new session ID while preserving ancestry via `parent_session_id`.
