...
The messaging gateway is the long-running process that connects IO to external platforms.
...
- `~/.io/gateway.json`
- selected bridged values from `~/.io/config.yaml`
...
Honcho tools (`honcho_profile`, `honcho_search`, `honcho_context`, `honcho_conclude`) need to execute against the correct user's Honcho session. In a multi-user gateway, the process-global module state in `tools/honcho_tools.py` is insufficient Φ multiple sessions may be active concurrently.

The solution threads session context through the call chain:

```
AIAgent._invoke_tool()
  Φ handle_function_call(honcho_manager=..., honcho_session_key=...)
    Φ registry.dispatch(**kwargs)
      Φ _handle_honcho_*(args, **kw)
        Φ _resolve_session_context(**kw)   # prefers explicit kwargs over module globals
```
