...
IO currently supports three API execution modes:
...
IO wraps API requests so they can be interrupted from the CLI or gateway.
...
IO uses two execution strategies:
...
IO tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, IO may:
