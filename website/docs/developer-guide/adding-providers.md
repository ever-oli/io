...
description: "How to add a new inference provider to IO Φ auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

IO can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `io model` menu entries
...
1. `io_cli/auth.py` decides how credentials are found.
2. `io_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `io_cli/models.py`, `io_cli/main.py`, and `io_cli/setup.py` make the provider show up in the CLI.
...
### Path A Φ OpenAI-compatible provider
...
- `PROVIDER_REGISTRY` in `io_cli/auth.py`
- `_PROVIDER_LABELS` in `io_cli/models.py`
- `_PROVIDER_ALIASES` in both `io_cli/auth.py` and `io_cli/models.py`
- CLI `--provider` choices in `io_cli/main.py`
...
## Step 2: Add auth metadata in `io_cli/auth.py`
...
- What env vars should IO check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `io_cli/models.py`
...
## Step 4: Resolve runtime data in `io_cli/runtime_provider.py`
...
Be careful with API-key precedence. IO already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `io_cli/main.py` and `io_cli/setup.py`
...
If you only update one of these files, `io model` and `io setup` will drift.
...
When you add a native provider, double-check that IO is only sending fields that provider actually understands.
...
```bash
source .venv/bin/activate
python -m io_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `io_cli/auth.py`
- [ ] aliases added in `io_cli/auth.py` and `io_cli/models.py`
- [ ] model catalog added in `io_cli/models.py`
- [ ] runtime branch added in `io_cli/runtime_provider.py`
- [ ] CLI wiring added in `io_cli/main.py`
- [ ] setup wiring added in `io_cli/setup.py`
...
### 7. Updating `io model` but not `io setup`
