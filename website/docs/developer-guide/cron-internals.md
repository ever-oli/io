...
description: "How IO stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

IO cron support is implemented primarily in:
...
Cron jobs are stored in IO-managed local state (`~/.io/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, IO loads those skills in order and then appends the job prompt as the task instruction.
...
IO uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
