...
description: "Reference for IO MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
IO may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, IO only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, IO does not create an empty MCP runtime toolset for that server.
