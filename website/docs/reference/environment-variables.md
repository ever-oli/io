...
description: "Complete reference of all environment variables used by IO"
---

# Environment Variables Reference

All variables go in `~/.io/.env`. You can also set them with `io config set VAR value`.
...
| `COPILOT_GITHUB_TOKEN` | GitHub token for Copilot API Φ first priority (OAuth `gho_*` or fine-grained PAT `github_pat_*`; classic PATs `ghp_*` are **not supported**) |
| `GH_TOKEN` | GitHub token Φ second priority for Copilot (also used by `gh` CLI) |
| `GITHUB_TOKEN` | GitHub token Φ third priority for Copilot |
| `IO_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `IO_COPILOT_ACP_COMMAND` |
| `IO_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `MINIMAX_API_KEY` | MiniMax API key Φ global endpoint ([minimax.io](https://www.minimax.io)) |
| `MINIMAX_BASE_URL` | Override MiniMax base URL (default: `https://api.minimax.io/v1`) |
| `MINIMAX_CN_API_KEY` | MiniMax API key Φ China endpoint ([minimaxi.com](https://www.minimaxi.com)) |
...
| `OPENCODE_ZEN_API_KEY` | OpenCode Zen API key Φ pay-as-you-go access to curated models ([opencode.ai](https://opencode.ai/auth)) |
| `OPENCODE_ZEN_BASE_URL` | Override OpenCode Zen base URL |
| `OPENCODE_GO_API_KEY` | OpenCode Go API key Φ $10/month subscription for open models ([opencode.ai](https://opencode.ai/auth)) |
...
| `IO_MODEL` | Preferred model name (checked before `LLM_MODEL`, used by gateway) |
| `LLM_MODEL` | Default model name (fallback when not set in config.yaml) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `IO_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `IO_LOCAL_STT_LANGUAGE` | Default language passed to `IO_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `IO_HOME` | Override IO config directory (default: `~/.io`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, IO prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.

| Variable | Description |
|----------|-------------|
| `IO_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `anthropic`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`, `kilocode`, `alibaba` (default: `auto`) |
| `IO_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override Nous inference API URL |
| `IO_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `IO_NOUS_TIMEOUT_SECONDS` | HTTP timeout for Nous credential / token flows |
| `IO_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `IO_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `IO_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.io/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@io:matrix.org`) Φ required for password login, optional with access token |
...
| `API_SERVER_HOST` | Host/bind address for the API server (default: `127.0.0.1`). Use `0.0.0.0` for network access Φ set `API_SERVER_KEY` for security. |
...
| Variable | Description |
|----------|-------------|
| `IO_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `IO_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `IO_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `IO_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `IO_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `IO_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `IO_QUIET` | Suppress non-essential output (`true`/`false`) |
| `IO_API_TIMEOUT` | LLM API call timeout in seconds (default: `900`) |
| `IO_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `IO_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `IO_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |
...
Context compression is configured exclusively through the `compression` section in `config.yaml` Φ there are no environment variables for it.
...
For task-specific direct endpoints, IO uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.

## Fallback Model (config.yaml only)

The primary model fallback is configured exclusively through `config.yaml` Φ there are no environment variables for it. Add a `fallback_model` section with `provider` and `model` keys to enable automatic failover when your main model encounters errors.
...
These go in `~/.io/config.yaml` under the `provider_routing` section:
...
:::tip
Use `io config set` to set environment variables Φ it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
