---
sidebar_position: 5
title: "Use MCP with IO"
description: "A practical guide to connecting MCP servers to IO, filtering their tools, and using them safely in real workflows"
---

# Use MCP with IO

This guide shows how to actually use MCP with IO in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native IO tool
- you want IO to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect IO to internal APIs, databases, or company systems without modifying IO core

Do not use MCP when:
- a built-in IO tool already solves the job well
...
- IO remains the agent
- MCP servers contribute tools
- IO discovers those tools at startup or reload time
- the model can use them like normal tools
- you control how much of each server is visible

That last part matters. Good MCP usage is not just Φconnect everything.Φ It is Φconnect the right thing, with the smallest useful surface.Φ

## Step 1: install MCP support

If you installed IO with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- IO banner/status should show MCP integration when configured
- ask IO what tools it has available
...
There are two categories of MCP-exposed functionality in IO:
...
So IO will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want IO to reason over a bounded workspace.
...
Now IO can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing IO core.
...
Because IO now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
