...
By the end, you'll have a fully automated workflow combining **web search**, **cron scheduling**, **delegation**, and **messaging delivery** Φ no code required.

## What We're Building

Here's the flow:

1. **8:00 AM** Φ The cron scheduler triggers your job
2. **IO spins up** a fresh agent session with your prompt
...
- **IO installed** Φ see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** Φ the gateway daemon handles cron execution:
  ```bash
  io gateway install   # Install as a user service
  sudo io gateway install --system   # Linux servers: boot-time system service
  # or
  io gateway           # Run in foreground
  ```
- **Firecrawl API key** Φ set `FIRECRAWL_API_KEY` in your environment for web search
- **Messaging configured** (optional but recommended) Φ [Telegram](/docs/user-guide/messaging/telegram) or Discord set up with a home channel

:::tip No messaging? No problem
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.io/cron/output/` and you can read them anytime.
:::
...
IO will search the web, read through results, and produce something like:

```
ΦΦ️ Your AI Briefing Φ March 8, 2026
...
1. Qwen 3 Released with 235B Parameters
   ...
   Φ https://qwenlm.github.io/blog/qwen3/
...
2. LangChain Launches Agent Protocol Standard
   ...
   Φ https://blog.langchain.dev/agent-protocol/
...
3. EU AI Act Enforcement Begins for General-Purpose Models
   ...
   Φ https://artificialintelligenceact.eu/updates/

---
3 stories Φ Sources searched: 8 Φ Generated by IO
```
...
IO will create the cron job for you using the unified `cronjob` tool.
...
Cron jobs run in a **completely fresh session** Φ no memory of your previous conversations, no context about what you "set up earlier." Your prompt must contain **everything** the agent needs to do the job.
:::
...
Topics:
1. AI and machine learning Φ focus on open source models and agent frameworks
2. Cryptocurrency Φ focus on Bitcoin, Ethereum, and regulatory news
3. Space exploration Φ focus on SpaceX, NASA, and commercial space
...
For faster briefings, tell IO to delegate each topic to a sub-agent:
...
Don't need briefings on weekends? Use a cron expression that targets MondayΦFriday:
...
If you have [memory](/docs/user-guide/features/memory) enabled, you can store preferences that persist across sessions. But remember Φ cron jobs run in fresh sessions without conversational memory. To add personal context, bake it directly into the prompt:
...
Search for the latest news on these topics. Summarize the top 3 stories with links. Be concise and technical Φ this reader doesn't need basic explanations."
```
...
IO will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
...
- **[Scheduled Tasks (Cron)](/docs/user-guide/features/cron)** Φ Full reference for schedule formats, repeat limits, and delivery options
- **[Delegation](/docs/user-guide/features/delegation)** Φ Deep dive into parallel sub-agent workflows
- **[Messaging Platforms](/docs/user-guide/messaging)** Φ Set up Telegram, Discord, or other delivery targets
- **[Memory](/docs/user-guide/features/memory)** Φ Persistent context across sessions
- **[Tips & Best Practices](/docs/guides/tips)** Φ More prompt engineering advice
