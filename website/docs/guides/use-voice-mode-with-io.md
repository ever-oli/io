---
sidebar_position: 7
title: "Use Voice Mode with IO"
description: "A practical guide to setting up and using IO voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with IO
...
- you want IO sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in IO.
...
## Step 1: make sure normal IO works first
...
Why these matter:
- `portaudio` Φ microphone input / playback for CLI voice mode
- `ffmpeg` Φ audio conversion for TTS and messaging delivery
- `opus` Φ Discord voice codec support
- `espeak-ng` Φ phonemizer backend for NeuTTS

## Step 4: choose STT and TTS providers

IO supports both local and cloud speech stacks.
...
- `local` Φ best default for privacy and zero-cost use
- `groq` Φ very fast cloud transcription
- `openai` Φ good paid fallback

#### Text-to-speech

- `edge` Φ free and good enough for most users
- `neutts` Φ free local/on-device TTS
- `elevenlabs` Φ best quality
- `openai` Φ good middle ground

### If you use `io setup`

If you choose NeuTTS in the setup wizard, IO checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
- asking IO to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full IO loop.

## Tuning CLI behavior

### Silence threshold

If IO starts/stops too aggressively, tune:
...
IO stays a normal chat bot, but can speak replies.
...
- you want IO to function like a portable research or ops assistant
...
IO joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- IO detects speech boundaries
- transcripts are posted in the associated text channel
- IO responds in text and audio
