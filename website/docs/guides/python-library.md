---
sidebar_position: 4
title: "Using IO as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines Φ no CLI required"
---

# Using IO as a Python Library

IO isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install IO directly from the repository:

```bash
pip install git+https://github.com/ever-oli/io.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+https://github.com/ever-oli/io.git
```

You can also pin it in your `requirements.txt`:

```text
io @ git+https://github.com/ever-oli/io.git
```

:::tip
The same environment variables used by the CLI are required when using IO as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use IO is the `chat()` method Φ pass a message, get a string back:
...
`chat()` handles the full conversation loop internally Φ tool calls, retries, everything Φ and returns just the final text response.

:::warning
Always set `quiet_mode=True` when embedding IO in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
- **`final_response`** Φ The agent's final text reply
- **`messages`** Φ The complete message history (system, user, assistant, tool calls)
- **`task_id`** Φ The task identifier used for VM isolation
...
# Second turn Φ agent remembers the context
...
Enable trajectory saving to capture conversations in ShareGPT format Φ useful for generating training data or debugging:
...
This is ideal for building specialized agents Φ a code reviewer, a documentation writer, a SQL assistant Φ all using the same underlying tooling.

---

## Batch Processing

For running many prompts in parallel, IO includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
- Set **`skip_memory=True`** to prevent the agent from reading or writing persistent memory Φ recommended for stateless API endpoints.
