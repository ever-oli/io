...
print_banner() {
    ...
    echo "│                    IO Installer                         │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI coding agent by ever-oli.            │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/ever-oli/io/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$IO_HOME/node/bin/node" ]; then
        export PATH="$IO_HOME/node/bin:$PATH"
        local found_ver=$("$IO_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (IO-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.io/node/..."
    ...
    # Place into ~/.io/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$IO_HOME/node"
    mkdir -p "$IO_HOME"
    mv "$extracted_dir" "$IO_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$IO_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$IO_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$IO_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$IO_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$IO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.io/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "IO itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="io-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if IO behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up io command..."

    if [ "$USE_VENV" = true ]; then
        IO_BIN="$INSTALL_DIR/venv/bin/io"
    else
        IO_BIN="$(which io 2>/dev/null || echo "")"
        if [ -z "$IO_BIN" ]; then
            log_warn "io not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$IO_BIN" ]; then
        log_warn "io entry point not found at $IO_BIN"
    ...
    log_success "Symlinked io → ~/.local/bin/io"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# IO — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so io works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "io command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.io directory structure (config at top level, code in subdir)
    mkdir -p "$IO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.io/.env (top level, easy to find)
    if [ ! -f "$IO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$IO_HOME/.env"
            log_success "Created ~/.io/.env from template"
        else
            touch "$IO_HOME/.env"
            log_success "Created ~/.io/.env"
        fi
    else
        log_info "~/.io/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.io/config.yaml (top level, easy to find)
    if [ ! -f "$IO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$IO_HOME/config.yaml"
            log_success "Created ~/.io/config.yaml from template"
        fi
    else
        log_info "~/.io/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$IO_HOME/SOUL.md" ]; then
        cat > "$IO_HOME/SOUL.md" << 'SOUL_EOF'
# IO Persona
...
-->
SOUL_EOF
        log_success "Created ~/.io/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.io/"

    # Seed bundled skills into ~/.io/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.io/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.io/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$IO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$IO_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.io/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — IO itself does not require root access."
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for IO to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$IO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'io whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                IO_CMD="$HOME/.local/bin/io"
                [ ! -x "$IO_CMD" ] && IO_CMD="io"
                $IO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'io whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'io gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $IO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $IO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: io gateway start"
                fi
            else
            ...
            log_info "To restart later: io gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: io gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.io/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.io/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.io/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.io/cron/, sessions/, logs/"
    ...
    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'io' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
