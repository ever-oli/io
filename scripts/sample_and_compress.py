...
DEFAULT_DATASETS = [
    ...
    "ever-oli/io-megascience-sft1",
    "NousResearch/IO-Thinking-GLM-4.7-SFT2",
    "NousResearch/IO-Thinking-GLM-4.7-SFT1",
    "NousResearch/terminal-tasks-glm-io"
]
...
def load_dataset_from_hf(dataset_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"   Î¦Î¦ ï¸  Error loading {dataset_name}: {e}")
        return []
    ...
    print(f"   Î¦œ… Loaded {len(entries):,} entries from {dataset_name}")
    return entries
...
def sample_from_datasets(
    ...
    for dataset_name in datasets:
        ...
        if not entries:
            print(f"   Î¦Î¦ ï¸  Skipping {dataset_name} (no entries loaded)")
            continue
    ...
    print(f"\n   Î¦œ… Found {len(filtered_entries):,} trajectories >= {min_tokens:,} tokens")
    ...
    if len(filtered_entries) <= total_samples:
        print(f"\nÎ¦Î¦ ï¸  Only {len(filtered_entries):,} trajectories available, using all of them")
        sampled = filtered_entries
    else:
        sampled = random.sample(filtered_entries, total_samples)
        print(f"\nÎ¦œ… Randomly sampled {len(sampled):,} trajectories from pool of {len(filtered_entries):,}")
...
def save_samples_for_compression(
    ...
    print(f"   Î¦œ… Saved {num_batches} batch files")
...
def merge_output_to_single_jsonl(input_dir: Path, output_file: Path):
    ...
    print(f"   Î¦œ… Merged {len(all_entries):,} entries into {output_file.name}")
    return output_file
...
def main(
    ...
    if not skip_download:
        ...
        if not samples:
            print("Î¦Œ No samples collected. Exiting.")
            return
        
        # Step 2: Save to JSONL files
        save_samples_for_compression(samples, sampled_dir, batch_size)
    else:
        print(f"\nÎ¦­ï¸  Skipping download, using existing data in {sampled_dir}")
    
    # Step 3: Run compression
    config_path = base_dir / config
    if not config_path.exists():
        print(f"Î¦Œ Config not found: {config_path}")
        return
